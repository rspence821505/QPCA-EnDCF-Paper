...
\]
where the eigenvalues are ordered as $\lambda_1\ge\cdots\ge\lambda_d\ge 0$ and $\hat{\lambda}_1\ge\cdots\ge\hat{\lambda}_d\ge 0$ (with the convention that $\hat{\lambda}_i=0$ for $i>\min(d,N{-}1)$). The population covariance $\boldsymbol{\Sigma}_E$ has rank at most $d$, while the sample covariance $\mathbf{C}_E$ has rank at most $\min(d,N-1)$ due to the centering step. For any truncation level $1\le\kappa\le \min(d,N-1)$, we define the corresponding truncated covariances
\[
...
\end{split}
\end{equation}
Here $C_{\mathrm{bias},1},C_{\mathrm{bias},2},C_{\mathrm{var}},C_{\mathrm{p}}>0$ depend only on $\|\mathbf{K}^{\mathrm{DC}}\|_2$ and $\|\mathbf{R}^{(L)}\|_2$ (and the inverse-stability constant from~\cite{ButlerWildeyZhang2020}), but are independent of $N$. Under Gaussian sampling, the projector-estimation contribution in \eqref{eq:kappa_proxy_objective} scales as $\mathcal{O}(1/N)$. Note that the map-approximation term $C_{\mathrm{bias},2}\|Q_s-Q\|_{L^2}^2$ and the forecast-fluctuation term $(C_{\mathrm{var}}/N)\,\mathrm{tr}(\boldsymbol{\Sigma}_E^{\,s})$ do not depend on $\kappa$ and therefore do not affect the minimizer; they are included only for completeness. In practice, $\kappa^\ast$ is determined by balancing the truncation remainder $\mathcal{T}_\kappa^{\,s}$ against the projector-estimation term $\kappa/\delta_\kappa^2$.
\end{corollary}
...
\[
\mathbb{E}\!\left[\|\hat{\mathbf{P}}^f-\mathbf{P}^f\|_F^2\right] = \mathcal{O}\!\left(\frac{\mathrm{tr}(\mathbf{P}^f)^2 + \|\mathbf{P}^f\|_F^2}{N}\right),
\qquad
...
\For{$w = 1$ to $W$}
    ...
    \State $\mathbf{A}_x \gets \mathbf{X}_L - \bar{\mathbf{x}}_L^f\mathbf{1}^{\top}$, \quad $\mathbf{A}_y \gets \mathbf{Y}_{\mathrm{stack}} - \bar{\mathbf{y}}_{\mathrm{stack}}\mathbf{1}^{\top}$ \Comment{Forecast anomalies}
