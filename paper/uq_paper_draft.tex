...
Windowed assimilation also amplifies sampling effects in stochastic ensemble methods. Observation perturbations must be drawn in the higher-dimensional space $\mathbb{R}^{d}$. Under the assumption that perturbations $\{\boldsymbol{\epsilon}_w^{(j)}\}_{j=1}^N$ are i.i.d.\ with $\boldsymbol{\epsilon}_w^{(j)}\sim\mathcal{N}(\mathbf{0},\mathbf{R}^{(L)})$ and $\mathbf{R}^{(L)}=\sigma_{\mathrm{obs}}^2\mathbf{I}_{d}$, consider the (unbiased) sample covariance
\[
\widehat{\mathbf{R}}^{(L)}
:=
...
=
\mathcal{O}\!\left(\sigma_{\mathrm{obs}}^2\sqrt{\frac{d}{N}}\right),
\]
and hence, by Jensen's inequality,
$\mathbb{E}\|\widehat{\mathbf{R}}^{(L)}-\mathbf{R}^{(L)}\|_F=\mathcal{O}(\sigma_{\mathrm{obs}}^2\sqrt{d/N})$. Relative to the sequential case ($d=m$), the prefactor grows like $\sqrt{L}$, so for moderate ensemble sizes this $\sqrt{L}$ amplification can offset gains from temporal coupling. Quantifying and mitigating this tradeoff between information gain and sampling noise motivates the comparative numerical analysis that follows.
...
\]
in agreement with the Kalman update formula, where $\widehat{\mathbf{P}}^{\,a}$ denotes the sample covariance of the analysis ensemble. Second, by presenting each ensemble member with a distinct realization of the observation, the perturbations prevent collapse of the ensemble onto a single point when the same observation is assimilated repeatedly. These benefits, however, come at the cost of additional sampling variability. The perturbation-induced contribution to the ensemble mean error has expected squared norm $\mathcal{O}(\|\mathbf{K}_k\mathbf{R}^{1/2}\|_F^2/N)$, which scales with the gain magnitude and observation error covariance. This sampling noise can substantially degrade both point estimates and uncertainty quantification for small ensemble sizes (see Section~\ref{subsec:spread_skill}).
...
For clarity, we describe the construction here at the algorithmic level; the corresponding population and sample objects (forecast distribution, stacked QoI map, whitened residual covariances, spectral projectors, and empirical gains) are defined precisely in Section~\ref{sec:theoretical_framework}.
...
This section introduces the mathematical framework and notation underlying the analysis of QPCA-EnDCF. We distinguish between population objects—deterministic quantities defined with respect to the forecast distribution—and sample objects computed from a finite ensemble and therefore inherently random. Population quantities represent the idealized limit of an infinite ensemble; sample quantities fluctuate around their population analogues due to finite-$N$ sampling effects. A primary goal of the theoretical results developed later is to quantify this discrepancy by establishing concentration bounds. All random quantities are defined on an underlying probability space $(\Omega,\mathcal{F},\mathbb{P})$, and all expectations are taken with respect to $\mathbb{P}$ unless otherwise stated. In the small-ensemble regime, the empirical observation-space covariance is necessarily singular, requiring a stabilized inverse (see Remark~\ref{rem:pyy_pinv_default}). We proceed by formalizing the forecast and observation spaces and the stacked QoI map, then defining whitened residual covariances and their spectra, and finally introducing the empirical EnDCF gain and QPCA projectors used by the algorithm.

\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Timing convention for four-dimensional assimilation).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Interpretation of the forecast distribution).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Convergence of approximate QoI maps).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Notation correspondence with prior work).}
...
The corresponding population mean and covariance, conditional on the realized observations $\mathbf{z}^{(w)}$, are given by
\begin{align*}
\boldsymbol{\mu}_E
&:=
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Conditioning convention).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Role of whitening).}
...
\subsection{Empirical EnDCF Covariances and Gain}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Why the pseudoinverse is the appropriate default).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Nonuniqueness under eigenvalue multiplicity).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Truncation and mean--innovation alignment).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Scope of the i.i.d.\ assumption).}
\label{rem:iid_scope}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Optional Gaussian case).}
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Role of spectral gaps).}
...
$\mathbf{K}^{\mathrm{DC}}=\mathbf{P}_{xy}\mathbf{P}_{yy}^{\dagger}$ as in Definition~\ref{def:endcf_gain_ensemble}. Let
\(
...
&\le
2\,\mathbb{E}\!\left[\|\mathbf{K}^{\mathrm{DC}}\|_2^2\right]\,
\|\mathbf{R}^{(L)}\|_2\,
...
\smallskip
\noindent Insert and subtract the population-truncated correction $(\mathbf{R}^{(L)})^{1/2}\mathbf{P}_\kappa^{\,s}\boldsymbol{\mu}_E^{\,s}$ in \eqref{eq:mean_update_projector_main}, take expectations, and use $\mathbb{E}[\bar{\mathbf{e}}^{\,s}]=\boldsymbol{\mu}_E^{\,s}$ to obtain
\[
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (When can truncation bias be controlled by an eigenvalue tail?).}
...
\end{split}
\end{equation}
If $\mathbf{R}^{(L)}=\bar r\,\mathbf{I}$, then $\mathrm{tr}(\mathbf{R}^{(L)})=d\,\bar r$ and the injected variance scales as
$\mathcal{O}(d/N)$.
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Interpreting Corollary~\ref{cor:stochastic_comparison_new}).}
Equation~\eqref{eq:stoch_var_bound_new} isolates an irreducible $\mathcal{O}(N^{-1})$ contribution to the variance of the stochastic analysis mean arising solely from observation perturbations. QPCA-EnDCF eliminates this specific source of sampling noise, but introduces variability through estimation of the low-rank projector; the latter contribution is controlled by the cutoff gap $\delta_\kappa$ and by the covariance concentration rate.
\par\smallskip
...
\par\smallskip\noindent\refstepcounter{remark}\textbf{Remark~\theremark\ (Metrics evaluation convention).}
\label{rem:metrics_convention}
For four-dimensional methods, analysis updates occur once per window (at window endpoints), yielding $W=50$ analysis states. For sequential methods, updates occur at every observation time, yielding $K=WL=250$ analysis states. To enable fair comparison of time-series calibration, spread and RMSE are evaluated at window endpoints only. We index these evaluation times by $w\in\{1,\ldots,W\}$, corresponding to global observation-time indices $k_w:=wL\in\{L,2L,\ldots,WL\}=\{5,10,\ldots,250\}$. Equations~\eqref{eq:spread_framework}--\eqref{eq:rmse_framework} therefore aggregate over $w=1,\ldots,W$. This convention ensures that (i) 4D methods are evaluated at the times when their analysis is actually computed, and (ii) sequential methods are evaluated at comparable temporal spacing, avoiding artifacts from dense versus sparse evaluation.
\par\smallskip
...
Sequential EnKF is strongly miscalibrated ($\chi^2=173{,}195$, flatness $1.861$), with a pronounced U shape: central ranks occur far more frequently than expected, while extreme ranks are depleted. This is the canonical signature of severe underdispersion caused by ensemble collapse. The 4D-EnKF shows improvement ($\chi^2=32{,}269$, flatness $1.796$) but retains a U-shaped structure, indicating persistent underdispersion despite temporal coupling. Increasing the effective dimension from $m=20$ to $mL=100$ without increasing ensemble size exacerbates this limitation.
...
The time-averaged Bias$^2$/MSE ratio (computed from the same window-wise estimates) further clarifies this distinction. For QPCA-EnDCF, the ratio is about $82\%$, indicating error dominated by systematic bias rather than sampling noise. In contrast, stochastic methods show ratios near $45$--$47\%$, implying that roughly half of their error arises from sampling variance at fixed ensemble size.
...
Multiplicative inflation rescales anomalies by a factor $\lambda_{\mathrm{infl}} \ge 1$,
\[
...
Additive inflation perturbs each ensemble member with independent noise,
\[
...
We perform a systematic parametric study to quantify inflation requirements across ensemble sizes and filtering methods. Ensemble size is varied over $N \in \{5,10,25,50\}$, multiplicative inflation over $\lambda_{\mathrm{infl}} \in \{1.0,1.05,1.10,1.15,1.20,1.25\}$, and additive inflation over $\alpha_{\mathrm{add}} \in \{0.0,0.01,0.05,0.10,0.15,0.20\}$, with all other parameters fixed at the baseline values of Section~\ref{sec:framework}. For additive inflation we use the standard isotropic choice $\mathbf{Q}_{\mathrm{add}}=\mathbf{I}_n$. Each configuration is repeated with three independent random seeds. Performance is assessed by time-averaged RMSE over the final 50 assimilation windows, after transients have decayed.
...
Here we test whether the performance advantages observed under Gaussian errors persist under non-Gaussian noise. The design isolates shape effects from covariance misspecification: every distribution is standardized to mean zero and variance $\sigma_{\mathrm{obs}}^2=2.25$, and all methods are supplied the correct covariance $\mathbf{R}=2.25\mathbf{I}_m$. Thus any performance changes reflect higher-order properties rather than incorrect second-moment information.
...
\caption{Representative observation error covariance structures $\mathbf{R}$ used in the correlation robustness study (left to right: exponential correlation, Gaussian correlation, and AR(1) correlation), constructed using periodic index distances to avoid boundary effects. All cases share the same marginal variance $\sigma_{\mathrm{obs}}^2=2.25$; colors indicate covariance magnitude.}
...
The bias-variance decomposition reveals that sampling variance scales as $\kappa/N$ (number of retained modes over ensemble size) rather than $d/N$ (observation dimension over ensemble size). For $\kappa = 1$ and $d = 100$, this represents theoretical order-of-magnitude sampling efficiency advantage. Truncation bias can be small when forecast errors concentrate in leading eigenmodes, a condition satisfied for chaotic systems near unstable manifolds. Empirical eigenvalue analyses confirm substantial concentration with the leading eigenvalue capturing 60--80\% of total residual variance (Section~\ref{subsec:benchmark}).
...
Joint processing of $L$ observations exploits temporal correlations through extended cross-covariance $\mathbf{P}_{xy} \in \mathbb{R}^{n \times d}$. Perturbations are sampled from $\mathcal{N}(\mathbf{0}, \mathbf{R}^{(L)})$ with block-diagonal covariance structure.
% ================================================================================
...
\For{$w = 1$ to $W$}
    ...
    \State \textit{// Update}
	    ...
	    \State $\mathbf{P}_{xy} \gets \frac{1}{N-1}\mathbf{A}_x\mathbf{A}_y^{\top}$, \quad $\mathbf{P}_{yy} \gets \frac{1}{N-1}\mathbf{A}_y\mathbf{A}_y^{\top}$
	    \State $\mathbf{K}^{\mathrm{DC}} \gets \mathbf{P}_{xy}(\mathbf{P}_{yy})^{\dagger}$
