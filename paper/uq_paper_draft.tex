...
% Improve line breaking to reduce overfull hboxes
...
\title{Spectral Regularization and Probabilistic Calibration in Ensemble Kalman Filtering: A Unified Theoretical Framework}
...
\begin{abstract}
Ensemble Kalman filtering in chaotic, partially observed systems is routinely performed with ensembles far smaller than the state dimension, where sampling noise and rank deficiency degrade both accuracy and probabilistic calibration. We develop a unified theoretical framework connecting spectral regularization in observation space to calibrated uncertainty quantification, and we analyze a deterministic alternative to stochastic observation perturbations: the Quantity-of-Interest Principal Component Analysis Ensemble Data Covariance Filter (QPCA-EnDCF). The method whitens forecast--observation residuals, identifies dominant mismatch modes via an empirical eigen-decomposition, and applies a rank-$\kappa$ truncated correction that leaves orthogonal noise-dominated directions unchanged, avoiding perturbation-induced variance injection. Our analysis separates population and finite-ensemble objects and combines covariance concentration with eigenspace perturbation bounds to obtain a bias--variance decomposition for the analysis mean. The results show that stochastic EnKF variants incur an irreducible variance term from perturbed observations that scales inversely with ensemble size, whereas QPCA-EnDCF replaces this with projector-estimation error controlled by spectral gaps and scales with retained rank rather than observation dimension. Numerical experiments on the Lorenz--96 system under severe undersampling (state dimension exceeding observation dimension exceeding ensemble size, with multi-step assimilation windows) confirm the theory: QPCA-EnDCF achieves near-ideal spread--skill ratios and strong temporal tracking between spread and error, while sequential and 4D stochastic EnKF remain strongly underdispersed despite inflation. The method simultaneously reduces RMSE, mitigates tuning requirements, and remains robust under non-ideal observation error assumptions, suggesting deterministic spectral regularization as a practical route to reliable uncertainty quantification in resource-constrained data assimilation.
\end{abstract}

\noindent\textbf{Keywords}: ensemble data assimilation, uncertainty quantification, spectral regularization, probabilistic calibration, chaotic dynamical systems
...
We focus on the Quantity-of-Interest Principal Component Analysis Ensemble Data Covariance Filter (QPCA-EnDCF), a deterministic alternative to perturbed-observation stochastic EnKF variants. QPCA-EnDCF whitens forecast--observation residuals, identifies dominant mismatch modes through an empirical spectral decomposition, and applies a rank-$\kappa$ truncated correction. This construction confines updates to signal-dominated subspaces while leaving orthogonal directions unchanged, reducing perturbation-induced sampling variance and yielding a controlled bias--variance tradeoff governed by the retained spectral rank.

Our evaluation emphasizes probabilistic calibration: ensembles should be reliable as distributions, not merely accurate as point estimators. We therefore assess spread--skill behavior, rank-based reliability diagnostics, and temporal tracking between ensemble spread and realized error. Controlled experiments on the Lorenz--96 system demonstrate that QPCA-EnDCF maintains near-calibrated uncertainty under severe undersampling, whereas conventional stochastic filters remain strongly underdispersed despite inflation.

The remainder of the paper is organized as follows. Section~\ref{sec:problem} specifies the dynamical and observational model and describes the assimilation algorithms. Section~\ref{sec:theoretical_framework} introduces the probabilistic and spectral framework used to analyze QPCA-EnDCF, and Section~\ref{sec:convergence_analysis} establishes convergence and bias--variance results. Sections~\ref{sec:framework}--\ref{sec:discussion} present numerical experiments and operational considerations, culminating in conclusions in Section~\ref{sec:conclusion}.
...
\begin{equation}
\frac{\mathrm{d}x_k}{\mathrm{d}t}
=
(x_{k+1}-x_{k-2})x_{k-1}-x_k+F,
\qquad k=1,\ldots,40,
\label{eq:lorenz96}
\end{equation}
with periodic indexing modulo $40$ and constant forcing $F=8.0$. The dynamics reflect the interaction of three components: a quadratic advection term $(x_{k+1}-x_{k-2})x_{k-1}$ that transfers energy across scales and induces strong nonlinearity, linear damping $-x_k$ that prevents unbounded growth, and constant forcing that maintains a statistically stationary, nonequilibrium regime.

% At $F=8.0$, the system is strongly chaotic, with approximately $13$ positive Lyapunov exponents. The largest exponent, $\lambda_{\max}\approx 1.68$, governs exponential error growth along the most unstable direction:
% \[
% \|\delta\mathbf{x}(t)\|\sim \mathrm{e}^{\lambda_{\max}t}\|\delta\mathbf{x}(0)\|.
% \]
% The associated Lyapunov timescale,
% \[
% \tau_{\mathrm{Lyap}}=\lambda_{\max}^{-1}\approx 0.6,
% \]
% characterizes the time over which infinitesimal perturbations amplify by a factor of $\mathrm{e}$. From an uncertainty quantification perspective, this rapid instability is central: forecast covariances must expand along unstable directions and contract along stable manifolds, while nonlinear stretching and folding induce anisotropy and departures from Gaussianity. Ensemble-based filters must therefore track a highly time-dependent and directional uncertainty structure to remain calibrated.

% Observations are assimilated at regular intervals $T_{\mathrm{obs}}=0.1$, corresponding to roughly six observation times per Lyapunov timescale. The nondimensional product
% \[
% \alpha=\lambda_{\max}T_{\mathrm{obs}}\approx 0.17
% \]
% measures the fraction of a Lyapunov time between observations, implying that forecast errors grow by a factor $\mathrm{e}^{\alpha}\approx 1.18$ between successive analyses. Although modest over a single interval, this growth compounds over many cycles, so even small systematic deficiencies in ensemble spread or covariance structure can accumulate and lead to loss of calibration or filter divergence.
...
To exploit temporal correlations, four-dimensional methods condition jointly on observations from multiple consecutive times. We stack $L$ observations within a window into
\begin{equation}
\mathbf{z}^{(w)}
=
\begin{bmatrix}
...
\end{equation}
where $w$ indexes the window and $(w-1)L+1,\ldots,wL$ denote the global observation time indices within that window. Assuming temporally independent observation errors, the augmented covariance is block diagonal, $\mathbf{R}^{(L)}=\mathbf{I}_L\otimes\mathbf{R}$. When the window length $LT_{\mathrm{obs}}$ is shorter than the Lyapunov timescale $\tau_{\mathrm{Lyap}}$, forecast ensembles retain substantial temporal correlation, allowing later observations within the window to inform the update of earlier or initial states through the joint conditioning. This smoothing-like capability is the principal advantage of four-dimensional formulations over purely sequential filters, which propagate information strictly forward in time.

In four-dimensional formulations it is convenient to view $\mathbf{z}^{(w)}$ as an element of a stacked observation space of dimension $d=mL$, and to define a corresponding stacked observation operator $\mathbf{H}^{(L)}$ mapping a window-initial state to predicted stacked observations; a tangent-linear version of this operator is introduced in Definition~\ref{def:data_space}.

Windowed assimilation also amplifies sampling effects in stochastic ensemble methods. Observation perturbations must be drawn in the higher-dimensional space $\mathbb{R}^{mL}$. Under the assumption that perturbations $\{\boldsymbol{\epsilon}_w^{(j)}\}_{j=1}^N$ are i.i.d.\ with $\boldsymbol{\epsilon}_w^{(j)}\sim\mathcal{N}(\mathbf{0},\mathbf{R}^{(L)})$ and $\mathbf{R}^{(L)}=\sigma_{\mathrm{obs}}^2\mathbf{I}_{mL}$, consider the (unbiased) sample covariance
\[
\widehat{\mathbf{R}}^{(L)}
:=
\frac{1}{N-1}\sum_{j=1}^N
(\boldsymbol{\epsilon}_w^{(j)}-\bar{\boldsymbol{\epsilon}}_w)
(\boldsymbol{\epsilon}_w^{(j)}-\bar{\boldsymbol{\epsilon}}_w)^\top,
\qquad
\bar{\boldsymbol{\epsilon}}_w:=\frac{1}{N}\sum_{j=1}^N \boldsymbol{\epsilon}_w^{(j)}.
\]
Then, with $d=mL$, one has (under finite fourth moments, and with explicit constants in the Gaussian case)
\[
...
=
\mathcal{O}\!\left(\sigma_{\mathrm{obs}}^2\sqrt{\frac{mL}{N}}\right),
\]
and hence, by Jensen's inequality,
$\mathbb{E}\|\widehat{\mathbf{R}}^{(L)}-\mathbf{R}^{(L)}\|_F=\mathcal{O}(\sigma_{\mathrm{obs}}^2\sqrt{mL/N})$. Relative to the sequential case ($d=m$), the prefactor grows like $\sqrt{L}$, so for moderate ensemble sizes this $\sqrt{L}$ amplification can offset gains from temporal coupling. Quantifying and mitigating this tradeoff between information gain and sampling noise motivates the comparative numerical analysis that follows.
...
When time indexing is relevant, we write $\mathbf{X}_k^{f}$ and $\mathbf{X}_k^{a}$ for the forecast and analysis ensembles at observation time $t_k$, and $\mathbf{x}_k^{(j),f}$ and $\mathbf{x}_k^{(j),a}$ for individual ensemble members. For windowed methods, it is sometimes necessary to distinguish window-initial ensembles $\mathbf{X}_0^{f}$ from window-endpoint ensembles $\mathbf{X}_L^{f}$, and we adopt this convention explicitly in the theoretical development (Remark~\ref{rem:timing_convention}).

Uncertainty is encoded by fluctuations about this mean. Defining the anomaly matrix
\begin{equation}
\mathbf{A}
=
...
\end{equation}
the empirical forecast covariance is
\begin{equation}
\widehat{\mathbf{P}}^{\,f}
=
\frac{1}{N-1}\mathbf{A}\mathbf{A}^\top.
\label{eq:sample_covariance}
\end{equation}
The factor $(N-1)^{-1}$ yields an unbiased estimator of the population covariance when ensemble members are independent draws from the forecast distribution. In sequential data assimilation, ensemble members become dependent over cycles; nevertheless, the $(N-1)^{-1}$ normalization remains customary and provides a finite-sample correction relative to the maximum-likelihood normalization $N^{-1}$.

The computational efficiency of ensemble methods comes with intrinsic limitations on uncertainty representation. In the small-ensemble regime $N\ll n$ typical of operational applications, $\widehat{\mathbf{P}}^{\,f}$ has rank at most $N-1$, causing filter divergence if unmitigated (see Section~\ref{sec:inflation} for detailed discussion). The algorithms examined in this work adopt distinct strategies to address these deficiencies while maintaining probabilistic calibration. Their differences lie primarily in how sampling noise is regularized and how ensemble variability is preserved to prevent collapse.



% ================================================================================

\subsection{Data Assimilation Algorithms}\label{subsec:algorithms_uq}
...
\subsubsection{Sequential Stochastic Ensemble Kalman Filter}
...
\mathbf{x}^{(j),f}
+
\mathbf{K}_k\bigl(\mathbf{z}_k+\boldsymbol{\epsilon}_{k}^{(j)}-\mathbf{H}\mathbf{x}^{(j),f}\bigr),
\qquad j=1,\ldots,N,
...
\mathbf{K}_k
=
\widehat{\mathbf{P}}^{\,f}\mathbf{H}^\top
\left(\mathbf{H}\widehat{\mathbf{P}}^{\,f}\mathbf{H}^\top+\mathbf{R}\right)^{-1}.
...
\[
\mathbb{E}_{\boldsymbol{\epsilon}}[\widehat{\mathbf{P}}^{\,a}]=(\mathbf{I}-\mathbf{K}_k\mathbf{H})\widehat{\mathbf{P}}^{\,f}(\mathbf{I}-\mathbf{K}_k\mathbf{H})^\top + \mathbf{K}_k\mathbf{R}\mathbf{K}_k^\top
\]
in agreement with the Kalman update formula, where $\widehat{\mathbf{P}}^{\,a}$ denotes the sample covariance of the analysis ensemble. Second, by presenting each ensemble member with a distinct realization of the observation, the perturbations prevent collapse of the ensemble onto a single point when the same observation is assimilated repeatedly. These benefits, however, come at the cost of additional sampling variability. The perturbation-induced contribution to the ensemble mean error has expected squared norm $\mathcal{O}(\|\mathbf{K}_k\mathbf{R}\|_F^2/N)$, which scales with the gain magnitude and observation error covariance. This sampling noise can substantially degrade both point estimates and uncertainty quantification for small ensemble sizes (see Section~\ref{subsec:spread_skill}).
...
\subsubsection{Four-Dimensional Stochastic Ensemble Kalman Filter}
...
\tilde{\mathbf{z}}^{(w,j)}
=
\mathbf{z}^{(w)}+\boldsymbol{\epsilon}_{w}^{(j)},
\qquad
\boldsymbol{\epsilon}_{w}^{(j)}\sim\mathcal{N}(\mathbf{0},\mathbf{R}^{(L)}),
\label{eq:4d_perturb}
\end{equation}
where $\mathbf{z}^{(w)}$ is the stacked observation vector defined in \eqref{eq:stacked_obs} and $\mathbf{R}^{(L)}$ is the corresponding block-diagonal covariance. The Kalman gain is computed analogously to the sequential case, but now in the augmented observation space $\mathbb{R}^{mL}$, relating the window-final state to the stacked observations through cross-covariances computed from the ensemble of forecasts at all $L$ times.
...
\subsubsection{Spectral Regularization Through QPCA-EnDCF}

The Quantity-of-Interest Principal Component Analysis Ensemble Data Covariance Filter (QPCA-EnDCF) adopts a fundamentally different strategy, eliminating observation perturbations altogether in favor of deterministic, spectrally regularized corrections. The method proceeds through three stages: whitening of forecast--observation residuals, spectral identification of dominant mismatch modes, and construction of truncated corrections.

For clarity, we describe the construction here at the algorithmic level; the corresponding population and sample objects (forecast law, stacked QoI map, whitened residual covariances, spectral projectors, and empirical gains) are defined precisely in Section~\ref{sec:theoretical_framework}.

In the first stage, residuals are normalized by observation uncertainty to obtain whitened mismatches. For the stacked four-dimensional observation framework with diagonal observation error covariance $\mathbf{R}^{(L)}=\sigma_{\mathrm{obs}}^2\mathbf{I}_{mL}$, this takes the form
\begin{equation}
\mathbf{E}
=
\sigma_{\mathrm{obs}}^{-1}
...
\end{equation}
where $\mathbf{Y}_{\mathrm{stack}}\in\mathbb{R}^{mL\times N}$ contains the stacked forecast observations for all ensemble members. The scalar form $\sigma_{\mathrm{obs}}^{-1}$ is used when $\mathbf{R}^{(L)}$ is diagonal; implementation details for non-diagonal covariances are provided in Section~\ref{subsec:correlated_errors}.
...
\end{equation}
where $r=\min(mL,N-1)$ denotes the maximum possible rank of $\mathbf{C}_E$, and $\mathbf{E}_c:=\mathbf{E}-\bar{\mathbf{e}}\mathbf{1}^\top$ denotes the centered whitened residual matrix, with $\mathbf{E}=[\mathbf{e}^{(1)},\ldots,\mathbf{e}^{(N)}]\in\mathbb{R}^{d\times N}$ and $\bar{\mathbf{e}}=\frac{1}{N}\sum_{j=1}^N\mathbf{e}^{(j)}$. The eigenvalues satisfy $\hat{\lambda}_1\geq\hat{\lambda}_2\geq\cdots\geq\hat{\lambda}_r\geq 0$, where the actual rank of $\mathbf{C}_E$ may be less than $r$ if some eigenvalues are zero (e.g., due to collinear ensemble members or degenerate configurations). In practice, the positive eigenvalues quantify the variance of forecast--observation mismatch along orthogonal directions $\hat{\mathbf{v}}_i$. Large eigenvalues correspond to coherent, dynamically meaningful discrepancies, whereas small (but nonzero) eigenvalues primarily reflect sampling and measurement noise.
...
This section introduces the mathematical framework and notation underlying the analysis of QPCA-EnDCF. We distinguish between \emph{population} objects—deterministic quantities defined with respect to the forecast law—and \emph{sample} objects computed from a finite ensemble and therefore inherently random. Population quantities represent the idealized limit of an infinite ensemble; sample quantities fluctuate around their population analogues due to finite-$N$ sampling effects. A primary goal of the theoretical results developed later is to quantify this discrepancy by establishing concentration bounds. All random quantities are defined on an underlying probability space $(\Omega,\mathcal{F},\mathbb{P})$, and all expectations are taken with respect to $\mathbb{P}$ unless otherwise stated. In the small-ensemble regime, the empirical observation-space covariance is necessarily singular, requiring a stabilized inverse (see Remark~\ref{rem:pyy_pinv_default}). We proceed by formalizing the forecast and observation spaces and the stacked QoI map, then defining whitened residual covariances and their spectra, and finally introducing the empirical EnDCF gain and QPCA projectors used by the algorithm. We denote by $n$ the state dimension and by $m$ the observed dimension per time. The assimilation window length is given by $L$, yielding a stacked observation dimension $d = mL$. Finally, $N$ denotes the ensemble size and $\kappa$ the spectral truncation rank. We use $k$ for the global observation-time index, $w$ for the window index, and $\ell\in\{1,\ldots,L\}$ for within-window indices. For windowed methods, we distinguish window-initial forecast ensembles $\mathbf{X}_0$ from window-endpoint forecast ensembles $\mathbf{X}_L$, and we write $\mathbf{Y}_{\mathrm{stack}}$ for stacked forecast observations.

\begin{remark}[Timing convention for four-dimensional assimilation]
...
\item \textbf{Window-endpoint states} $\mathbf{x}_L^{(j),f}$: These are the forecast states at the end of the window ($\ell=L$), obtained by propagating $\mathbf{x}_0^{(j),f}$ through $L$ model steps. The EnDCF gain $\mathbf{K}^{\mathrm{EnDCF}}$ (Definition~\ref{def:endcf_gain_ensemble}) maps observation-space corrections to state-space increments that are applied to $\mathbf{x}_L^{(j),f}$ to produce the analysis $\mathbf{x}_L^{(j),a}$.
\end{enumerate}
...
\begin{remark}[Interpretation of the forecast law]
...
\begin{assumption}[Gaussian forecast model (when invoked)]
...
\[
P_{\mathcal{X}}=\mathcal{N}(\boldsymbol{\mu}^{f},\mathbf{P}^{f}),
\]
so that $\mathbb{E}[\mathbf{x}^{(j),f}]=\boldsymbol{\mu}^{f}$ and $\mathrm{Cov}(\mathbf{x}^{(j),f})=\mathbf{P}^{f}$.
...
\in\mathbb{R}^{mL},
\]
where $\mathbf{M}_\ell\in\mathbb{R}^{n\times n}$ is the tangent linear model (TLM) obtained by linearizing the nonlinear dynamics $\mathcal{M}$ about a reference trajectory $\{\bar{\mathbf{x}}_\ell\}_{\ell=0}^{L}$ (typically the ensemble mean trajectory), so that $\mathbf{M}_\ell := \frac{\partial}{\partial\mathbf{x}_0}\mathcal{M}^{(\ell)}(\mathbf{x}_0)\big|_{\mathbf{x}_0=\bar{\mathbf{x}}_0}$ maps perturbations at the window-initial time to perturbations at time $\ell$. This tangent-linear formulation permits closed-form expressions for population covariance propagation and underpins the spectral analysis in Sections~\ref{sec:theoretical_framework}--\ref{sec:convergence_analysis}. The approximation is exact for linear dynamics and linear observation operators; for nonlinear dynamics, it is valid to first order in perturbations about the reference trajectory. The numerical experiments (Section~\ref{sec:framework}) propagate the full nonlinear model $\mathcal{M}$ and then apply the linear observation operator $\mathbf{H}$; the theoretical results thus characterize algorithm behavior in the tangent-linear regime, which provides a useful approximation when ensemble spread is small relative to the scale of nonlinearity.
\end{definition}

\begin{remark}[Convergence of approximate QoI maps]
\label{rem:map_convergence}
...
\begin{remark}[Notation correspondence with prior work]
\label{rem:notation_correspondence}
...
\subsection{Population and Sample Covariance Structures}
\label{subsec:population_sample_cov}
...
Let $\mathbf{R}^{(L)}\in\mathbb{R}^{d\times d}$ be symmetric positive definite. Consider a fixed realization of the observation vector $\mathbf{z}^{(w)}\in\mathbb{R}^{d}$, which we treat as deterministic throughout this definition. For each ensemble member $j$, let $\mathbf{x}_0^{(j),f}$ denote the window-initial forecast state, and define the whitened forecast--observation residual
\[
\mathbf{e}^{(j)}
:=
(\mathbf{R}^{(L)})^{-1/2}
\bigl(\mathbf{H}^{(L)}\mathbf{x}_0^{(j),f}-\mathbf{z}^{(w)}\bigr)
\in\mathbb{R}^{d},
\]
where $\mathbf{H}^{(L)}$ is the linear stacked observation operator from Definition~\ref{def:data_space}.
The corresponding population mean and covariance, \emph{conditional on the realized observations $\mathbf{z}^{(w)}$}, are given by
\begin{align*}
\boldsymbol{\mu}_E
&:=
...
=
(\mathbf{R}^{(L)})^{-1/2}
\bigl(\mathbf{H}^{(L)}\boldsymbol{\mu}^{f}-\mathbf{z}^{(w)}\bigr),\\
\boldsymbol{\Sigma}_E
&:=
...
Let $d:=mL$. The population covariance $\boldsymbol{\Sigma}_E$ and the sample covariance $\mathbf{C}_E$ admit spectral decompositions
\[
\boldsymbol{\Sigma}_E
=
...
\mathbf{C}_E
=
\sum_{i=1}^{d}\hat{\lambda}_i\,\hat{\mathbf{v}}_i\hat{\mathbf{v}}_i^\top,
\]
where the eigenvalues are ordered as $\lambda_1\ge\cdots\ge\lambda_d\ge 0$ and $\hat{\lambda}_1\ge\cdots\ge\hat{\lambda}_d\ge 0$. The population covariance $\boldsymbol{\Sigma}_E$ has rank at most $d$, while the sample covariance $\mathbf{C}_E$ has rank at most $\min(d,N-1)$ due to the centering step. For any truncation level $1\le\kappa\le \min(d,N-1)$, we define the corresponding truncated covariances
\[
...
\subsection{Empirical EnDCF Covariances and Gain (Pseudoinverse Form)}
\label{subsec:endcf_gain}

We now define the empirical covariance operators and gain matrix used by EnDCF in practice. These quantities are computed directly from finite ensembles and are therefore random. Particular care is required in the small-ensemble regime, where observation-space covariances are singular and classical inverses are not available.

\begin{definition}[EnDCF ensemble covariances and empirical gain]
\label{def:endcf_gain_ensemble}
Let $\mathbf{X}_0\in\mathbb{R}^{n\times N}$ denote the window-initial forecast ensemble, and let $\mathbf{X}_L\in\mathbb{R}^{n\times N}$ denote the forecast ensemble at the window endpoint (assimilation time). For four-dimensional methods, $\mathbf{X}_L$ is obtained by propagating $\mathbf{X}_0$ forward through $L$ observation times; for sequential methods, $\mathbf{X}_L=\mathbf{X}_0$. Let $\mathbf{Y}_{\mathrm{stack}}\in\mathbb{R}^{d\times N}$ denote the corresponding stacked forecast observations,
\[
...
\bar{\mathbf{x}}_L^{f}
:=
\frac{1}{N}\mathbf{X}_L\mathbf{1}\in\mathbb{R}^{n},
\qquad
...
\mathbf{A}_x
:=
\mathbf{X}_L-\bar{\mathbf{x}}_L^{f}\mathbf{1}^\top
...
\mathbf{P}_{xy}
:=
\frac{1}{N-1}\mathbf{A}_x\mathbf{A}_y^\top
\in\mathbb{R}^{n\times d},
\qquad
\mathbf{P}_{yy}
:=
\frac{1}{N-1}\mathbf{A}_y\mathbf{A}_y^\top
\in\mathbb{R}^{d\times d}.
\]
The EnDCF gain used in implementation is the \emph{empirical} gain
\begin{equation}
\mathbf{K}^{\mathrm{EnDCF}}
:=
\mathbf{P}_{xy}\,\mathbf{P}_{yy}^{\dagger}
\in\mathbb{R}^{n\times d},
...
\begin{remark}[Why the pseudoinverse is the appropriate default]
\label{rem:pyy_pinv_default}
The matrix $\mathbf{P}_{yy}$ has rank at most $\min(d,N-1)$ and is therefore singular whenever $N-1<d$, which is the regime of primary interest in this work. In such cases, the gain \eqref{eq:endcf_gain_pinv} remains well defined, whereas the inverse $\mathbf{P}_{yy}^{-1}$ does not exist. More generally, one may replace $\mathbf{P}_{yy}^{\dagger}$ with a stabilized inverse, such as Tikhonov regularization $(\mathbf{P}_{yy}+\varepsilon\mathbf{I})^{-1}$. All subsequent arguments remain valid provided the resulting gain is uniformly bounded in operator norm, as formalized below.
\end{remark}
...
\[
\mathbb{E}\!\left[\|\mathbf{K}^{\mathrm{EnDCF}}\|_2^{2}\right]<\infty.
\]
This moment condition is mild in practice: when a stabilized inverse such as $(\mathbf{P}_{yy}+\varepsilon\mathbf{I})^{-1}$ is used, the inverse is uniformly bounded, and the condition follows from standard moment bounds on the ensemble anomalies (e.g., if the forecast distribution has exponential or sub-exponential tails, as in the Gaussian case). The pseudoinverse satisfies the condition under similar regularity conditions on the forecast distribution. This is the only property of the gain required to pass from conditional to unconditional mean-square bounds in the analysis.
\end{assumption}

\begin{remark}[Scope of the pseudoinverse formulation]
\label{rem:pinv_in_main_results}
All results in this section are stated for the pseudoinverse-based gain $\mathbf{K}^{\mathrm{EnDCF}}=\mathbf{P}_{xy}\mathbf{P}_{yy}^{\dagger}$ introduced in Definition~\ref{def:endcf_gain_ensemble}. The same proofs apply verbatim to any stabilized inverse (e.g.\ ridge regularization) provided the resulting gain satisfies Assumption~\ref{ass:gain_moment}.
\end{remark}
...
\subsection{Spectral Projectors and QPCA Truncation}
...
% ================================================================================
\section{Convergence Analysis for QPCA-EnDCF}
...
\item The forecast law $P_{\mathcal{X}}$ has a finite second moment, that is,
\[
...
\begin{remark}[Scope of the i.i.d.\ assumption]
\label{rem:iid_scope}
The i.i.d.\ sampling assumption in (iii) is a mathematical idealization adopted for deriving the convergence bounds and concentration estimates in Sections~\ref{subsec:fundamental_concentration}--\ref{subsec:main_bv}. In practical cycling experiments (Sections~\ref{sec:calibration}--\ref{sec:operational}), the analysis ensemble from window $w$ becomes the forecast ensemble for window $w+1$, inducing temporal dependence that violates strict i.i.d.\ conditions. Nonetheless, the theoretical results provide qualitative guidance: under ergodic dynamics and sufficient mixing, consecutive windows become approximately independent in distribution, and the main conclusions---that sampling variance scales as $\mathcal{O}(1/N)$ and is controlled by $\kappa$ rather than $d$---remain empirically valid. The experiments demonstrate that the predicted performance advantages persist under cycling, even though the formal assumptions are satisfied only asymptotically or approximately.
\end{remark}
...
\(
\mathbf{v}:=\bar{\mathbf{x}}^{a}-\mathbb{E}[\bar{\mathbf{x}}^{a}],
\)
so that $\mathbb{E}[\mathbf{v}]=\mathbf{0}$. Writing
\(
\bar{\mathbf{x}}^{a}-\mathbf{x}^{\mathrm{true}}=\mathbf{b}+\mathbf{v},
\)
we compute
\[
\mathbb{E}\|\mathbf{b}+\mathbf{v}\|^2
=
\|\mathbf{b}\|^2
+
2\langle \mathbf{b},\mathbb{E}\mathbf{v}\rangle
+
\mathbb{E}\|\mathbf{v}\|^2
=
\|\mathbf{b}\|^2+\mathbb{E}\|\mathbf{v}\|^2,
\]
...
\begin{remark}[Role of Bessel's correction]
The normalization by $(N-1)$ compensates for the loss of one degree of freedom incurred by centering with the sample mean, ensuring unbiasedness of the covariance estimator for all $N\ge 2$.
\end{remark}
...
\]
	...
	\)
	and the standardized fourth-moment control implicit in the constant $C_{\mathrm{cov}}$ ensures that $\mathbb{E}\|\tilde{\mathbf{e}}^{(1)}\|^4$ is bounded by a dimension-dependent multiple of $\mathrm{tr}(\boldsymbol{\Sigma}_E^2)$. Absorbing this ratio into $C_{\mathrm{cov}}$ yields the stated bound.
\end{proof}

\begin{remark}[Optional Gaussian specialization]
If, in addition, $\tilde{\mathbf{e}}^{(1)}$ is Gaussian, one may take an explicit constant $C_{\mathrm{cov}}=8(d+2)$ via Isserlis' theorem, recovering the familiar Wishart-rate bound.
\end{remark}
...
\begin{enumerate}[label=(\roman*)]
\item \textbf{Eigenvalue perturbation (Weyl).}
...
\item \textbf{One-dimensional eigenspace perturbation (Davis--Kahan).}
Fix $i\in\{1,\ldots,d\}$ and assume that $\lambda_i$ is isolated, i.e.,
\[
\mathrm{gap}_i
:=
...
\le
\frac{2\,\|\mathbf{C}_E-\boldsymbol{\Sigma}_E\|_2}{\delta_i},
\qquad
\|\hat{\mathbf{v}}_i-\mathbf{v}_i\|_2
\le
\frac{4\,\|\mathbf{C}_E-\boldsymbol{\Sigma}_E\|_2}{\delta_i}.
\]

\item \textbf{Projector perturbation under a cutoff gap.}
Fix $\kappa\in\{1,\ldots,d\}$ and assume a cutoff gap
\[
...
\le
\frac{2\sqrt{2\kappa}}{\delta_\kappa}\,
\|\mathbf{C}_E-\boldsymbol{\Sigma}_E\|_2
\le
\frac{2\sqrt{2\kappa}}{\delta_\kappa}\,
...
\item \textbf{Truncated covariance error (projector-based form).}
With $\boldsymbol{\Sigma}_E^\kappa:=\mathbf{P}_\kappa\boldsymbol{\Sigma}_E\mathbf{P}_\kappa$
...
\smallskip
\noindent\emph{Step 1: Eigenvalue perturbation (Weyl).}
...
\smallskip
\noindent\emph{Step 2: Eigenspace perturbation (Davis--Kahan).}
Fix $i$ and assume the separation condition in (ii). The Davis--Kahan $\sin\Theta$ theorem gives
\[
\sin\angle(\hat{\mathbf{v}}_i,\mathbf{v}_i)
=
\|(\mathbf{I}-\mathbf{v}_i\mathbf{v}_i^\top)\hat{\mathbf{v}}_i\|_2
...
$\|\hat{\mathbf{v}}_i-\mathbf{v}_i\|_2\le 2\sin\angle(\hat{\mathbf{v}}_i,\mathbf{v}_i)$ yields the Euclidean bound in (ii).

\smallskip
\noindent\emph{Step 3: Cutoff-gap projector perturbation.}
Under the cutoff-gap condition in (iii), Davis--Kahan in projector form yields
\[
\|\hat{\mathbf{P}}_\kappa-\mathbf{P}_\kappa\|_F
\le
\frac{2\sqrt{2\kappa}}{\delta_\kappa}\,
...
\smallskip
\noindent\emph{Step 4: Truncated covariance error.}
Using $\boldsymbol{\Sigma}_E^\kappa=\mathbf{P}_\kappa\boldsymbol{\Sigma}_E\mathbf{P}_\kappa$ and
...
\smallskip
\noindent\emph{Step 1: Reduction to a central Wishart form.}
Since $\mathbf{J}_N$ is an orthogonal projector of rank $N-1$, there exists an orthogonal matrix
...
The key point is that right multiplication by an orthogonal matrix preserves the law of a Gaussian data matrix. Indeed, since the columns of $\mathbf{A}$ are i.i.d.\ $\mathcal{N}(\mathbf{0},\boldsymbol{\Sigma}_E)$, we have
\[
\mathbf{A}\sim MN_{d\times N}\!\left(\mathbf{0},\,\boldsymbol{\Sigma}_E,\,\mathbf{I}_N\right),
\]
equivalently $\mathrm{vec}(\mathbf{A})\sim \mathcal{N}\!\left(\mathbf{0},\,\mathbf{I}_N\otimes \boldsymbol{\Sigma}_E\right)$.
For any orthogonal $\mathbf{U}$,
\[
...
\]
so $\mathbf{A}\mathbf{U}\stackrel{d}{=}\mathbf{A}$. In particular, the columns of $\mathbf{A}\mathbf{U}$ are again i.i.d.\ $\mathcal{N}(\mathbf{0},\boldsymbol{\Sigma}_E)$, and thus the first $N-1$ columns
\[
...
\smallskip
\noindent\emph{Step 2: Wishart fourth-moment bound.}
Standard fourth-moment bounds for the Wishart distribution imply that there exists a constant $C_{\mathrm{W}}>0$, depending only on $d$, such that
\[
\mathbb{E}\!\left[\|\mathbf{C}_E-\boldsymbol{\Sigma}_E\|_F^4\right]
\le
\frac{C_{\mathrm{W}}}{(N-1)^2}\,\bigl(\mathrm{tr}(\boldsymbol{\Sigma}_E^2)\bigr)^2,
...
\begin{remark}[Role of spectral gaps]
Spectral separation is intrinsic to eigenspace stability: when eigenvalues are repeated or nearly repeated, arbitrarily small perturbations may induce large rotations within the associated invariant subspace even though eigenvalues remain stable. For QPCA-EnDCF, stability of the \emph{rank-$\kappa$ projector} is therefore strongest when the cutoff gap $\lambda_\kappa-\lambda_{\kappa+1}$ is well separated.
\end{remark}
...
\begin{theorem}[Bias--variance decomposition with spectral truncation]
\label{thm:bias_variance}
Consider QPCA-EnDCF with truncation level $\kappa$ and empirical gain
$\mathbf{K}^{\mathrm{EnDCF}}=\mathbf{P}_{xy}\mathbf{P}_{yy}^{\dagger}$ as in Definition~\ref{def:endcf_gain_ensemble}. Let
\(
...
&\le
2\,\mathbb{E}\!\left[\|\mathbf{K}^{\mathrm{EnDCF}}\|_2^2\right]\,
\|\mathbf{R}^{(L)}\|_2\,
...
\end{align}
for constants $C_{\mathrm{map}},C_{\mathrm{proj}}>0$ independent of $N$.

The variance satisfies
\begin{align}
\mathrm{Var}(N,\kappa,s)
&\le
\frac{2}{N}\,\mathbb{E}\!\left[\|\mathbf{x}^{(1),f}-\boldsymbol{\mu}^f\|^2\right]
+
\frac{2\|\mathbf{R}^{(L)}\|_2}{N}\,
\mathbb{E}\!\left[\|\mathbf{K}^{\mathrm{EnDCF}}\|_2^2\right]\,
\mathrm{tr}\!\bigl(\boldsymbol{\Sigma}_E^{\,s}\bigr)
\nonumber\\
&\hspace{3.2em}
+\;
2\|\mathbf{R}^{(L)}\|_2\,
\mathbb{E}\!\left[\|\mathbf{K}^{\mathrm{EnDCF}}\|_2^2\,
\|(\hat{\mathbf{P}}_\kappa^{\,s}-\mathbf{P}_\kappa^{\,s})\,(\bar{\mathbf{e}}^{\,s}-\boldsymbol{\mu}_E^{\,s})\|^2\right],
\label{eq:var_bound_fixed_main_raw}
\end{align}
In particular, by Cauchy--Schwarz and Lemma~\ref{lem:covariance_concentration}(iii) applied with $(\boldsymbol{\Sigma}_E,\mathbf{C}_E)$ replaced by $(\boldsymbol{\Sigma}_E^{\,s},\mathbf{C}_E^{\,s})$,
\begin{align}
...
\smallskip
\noindent\emph{Step 1: Projector form of the mean update.}
By the QPCA-EnDCF construction (Definition~\ref{def:endcf_gain_ensemble} and Definition~\ref{def:projectors_truncation}), the analysis update acts on the whitened residuals through the empirical rank-$\kappa$ projector:
\[
\mathbf{x}^{(j),a,s}
=
\mathbf{x}^{(j),f}
-
\mathbf{K}^{\mathrm{EnDCF}}(\mathbf{R}^{(L)})^{1/2}\hat{\mathbf{P}}_\kappa^{\,s}\,\mathbf{e}^{(j),s}.
\]
...
\bar{\mathbf{x}}^{f}
-
\mathbf{K}^{\mathrm{EnDCF}}(\mathbf{R}^{(L)})^{1/2}\hat{\mathbf{P}}_\kappa^{\,s}\,\bar{\mathbf{e}}^{\,s}.
\end{equation}

\smallskip
\noindent\emph{Step 2: Bias bound.}
Insert and subtract the population-truncated correction $(\mathbf{R}^{(L)})^{1/2}\mathbf{P}_\kappa^{\,s}\boldsymbol{\mu}_E^{\,s}$ in \eqref{eq:mean_update_projector_main}, take expectations, and use $\mathbb{E}[\bar{\mathbf{e}}^{\,s}]=\boldsymbol{\mu}_E^{\,s}$ to obtain
\[
\mathbb{E}[\bar{\mathbf{x}}^{a,s}]
=
\boldsymbol{\mu}^f
-
\mathbb{E}\!\left[\mathbf{K}^{\mathrm{EnDCF}}(\mathbf{R}^{(L)})^{1/2}\mathbf{P}_\kappa^{\,s}\,\boldsymbol{\mu}_E^{\,s}\right]
-
\mathbb{E}\!\left[\mathbf{K}^{\mathrm{EnDCF}}(\mathbf{R}^{(L)})^{1/2}(\hat{\mathbf{P}}_\kappa^{\,s}-\mathbf{P}_\kappa^{\,s})\,\boldsymbol{\mu}_E^{\,s}\right].
\]
The truncation remainder relative to the full population correction is governed by $(\mathbf{I}-\mathbf{P}_\kappa^{\,s})\boldsymbol{\mu}_E^{\,s}$. Using $\|ABc\|\le \|A\|_2\|B\|_2\|c\|$ and Cauchy--Schwarz yields
\[
\left\|\mathbb{E}\!\left[\mathbf{K}^{\mathrm{EnDCF}}(\mathbf{R}^{(L)})^{1/2}(\mathbf{I}-\mathbf{P}_\kappa^{\,s})\boldsymbol{\mu}_E^{\,s}\right]\right\|^2
\le
\mathbb{E}\|\mathbf{K}^{\mathrm{EnDCF}}\|_2^2\,\|\mathbf{R}^{(L)}\|_2\,\|(\mathbf{I}-\mathbf{P}_\kappa^{\,s})\boldsymbol{\mu}_E^{\,s}\|^2,
\]
which yields the leading truncation term in \eqref{eq:bias_bound_fixed_main} after a factor-$2$ slack to accommodate the remaining contributions.

The projector-estimation term is bounded similarly:
\[
\left\|\mathbb{E}\!\left[\mathbf{K}^{\mathrm{EnDCF}}(\mathbf{R}^{(L)})^{1/2}(\hat{\mathbf{P}}_\kappa^{\,s}-\mathbf{P}_\kappa^{\,s})\boldsymbol{\mu}_E^{\,s}\right]\right\|^2
\le
\mathbb{E}\|\mathbf{K}^{\mathrm{EnDCF}}\|_2^2\,\|\mathbf{R}^{(L)}\|_2\,
...
\)
and absorbing $\|\boldsymbol{\mu}_E^{\,s}\|^2$ into the constant yields the third term in \eqref{eq:bias_bound_fixed_main}.

Finally, the map-approximation term $C_{\mathrm{map}}\|Q_s-Q\|_{L^2(P_{\mathcal{X}})}^2$ follows from stability of the mean update with respect to $Q_s\to Q$ in $L^2(P_{\mathcal{X}})$ (cf.\ the continuity framework in~\cite{ButlerWildeyZhang2020}); we record it separately because it is orthogonal to the sampling and truncation mechanisms.

\smallskip
\noindent\emph{Step 3: Variance bound.}
Subtract $\mathbb{E}[\bar{\mathbf{x}}^{a,s}]$ from \eqref{eq:mean_update_projector_main} and apply $\|u+v\|^2\le 2\|u\|^2+2\|v\|^2$ to separate (i) the fluctuation of the forecast mean and (ii) the fluctuation of the projected residual term. Using the standard identities
\[
\mathbb{E}\|\bar{\mathbf{x}}^f-\boldsymbol{\mu}^f\|^2=\frac{1}{N}\mathbb{E}\|\mathbf{x}^{(1),f}-\boldsymbol{\mu}^f\|^2,
\qquad
...
\begin{remark}[When can truncation bias be controlled by an eigenvalue tail?]
...
\)
that is, on the component of the \emph{mean whitened innovation} discarded by truncation. Bounds of the form
\(
...
\left(\mathbb{E}\|\mathbf{e}^{(1),s}-\boldsymbol{\mu}_E^{\,s}\|^4+\bigl(\mathbb{E}\|\mathbf{e}^{(1),s}-\boldsymbol{\mu}_E^{\,s}\|^2\bigr)^2\right)^{1/2}.
\]
...
Assume the hypotheses of Theorem~\ref{thm:bias_variance}. Consider a stochastic windowed EnKF update with perturbed observations
\[
...
\]
where $\mathbf{K}^{(w)}$ is deterministic, or more generally independent of $\{\boldsymbol{\epsilon}_{w}^{(j)}\}_{j=1}^N$.
...
\end{split}
\end{equation}
If $\mathbf{R}^{(L)}=\bar r\,\mathbf{I}$, then $\mathrm{tr}(\mathbf{R}^{(L)})=mL\,\bar r$ and the injected variance scales as
$\mathcal{O}(mL/N)$.

By contrast, QPCA-EnDCF avoids this perturbation-induced variance source and instead satisfies
$\mathrm{Var}(N,\kappa,s)=\mathcal{O}(1/N)$ (for fixed approximation stage $s$), with an additional stability factor $\kappa/\delta_\kappa^2$ arising from estimation of the rank-$\kappa$ projector (cf.~\eqref{eq:var_projector_term_bound_main}).
\end{corollary}
...
\begin{remark}[Interpreting Corollary~\ref{cor:stochastic_comparison_new}]
Equation~\eqref{eq:stoch_var_bound_new} isolates an \emph{irreducible} $\mathcal{O}(N^{-1})$ contribution to the variance of the stochastic analysis mean arising solely from observation perturbations. QPCA-EnDCF eliminates this specific source of sampling noise, but introduces variability through estimation of the low-rank projector; the latter contribution is controlled by the cutoff gap $\delta_\kappa$ and by the covariance concentration rate.
\end{remark}

%--------------------------------------------------------------------
\begin{corollary}[Principled selection of the truncation level]
\label{cor:optimal_kappa_new}
Assume the hypotheses of Theorem~\ref{thm:bias_variance}. Define the deterministic truncation remainder
\[
\mathcal{T}_\kappa^{\,s}:=\|(\mathbf{I}-\mathbf{P}_\kappa^{\,s})\boldsymbol{\mu}_E^{\,s}\|^2,
\qquad
\boldsymbol{\mu}_E^{\,s}=\mathbb{E}\!\left[(\mathbf{R}^{(L)})^{-1/2}\bigl(Q_s(\mathbf{x}^f)-\mathbf{z}^{(w)}\bigr)\right],
\]
and let $\delta_\kappa=\lambda_\kappa^{\,s}-\lambda_{\kappa+1}^{\,s}$ denote the cutoff gap. A natural upper-bound proxy for selecting $\kappa$ is to minimize a bound of the form
\begin{equation}
\label{eq:kappa_proxy_objective}
\begin{split}
\kappa^\ast
&\in
\argmin_{1\le \kappa<\min(mL,N-1)}
\Bigl\{
C_{\mathrm{bias},1}\,\mathcal{T}_\kappa^{\,s}
+
C_{\mathrm{bias},2}\,\|Q_s-Q\|_{L^2(P_{\mathcal{X}})}^2\\
&\qquad\qquad
+
\frac{C_{\mathrm{var}}}{N}\,
\mathrm{tr}\!\bigl(\boldsymbol{\Sigma}_E^{\,s}\bigr)
+
C_{\mathrm{proj}}\,
\frac{\kappa}{\delta_\kappa^2}\,
\Bigl(\mathbb{E}\|\mathbf{C}_E^{\,s}-\boldsymbol{\Sigma}_E^{\,s}\|_F^4\Bigr)^{1/2}
\Bigl(\mathbb{E}\|\mathbf{e}^{(1),s}-\boldsymbol{\mu}_E^{\,s}\|^4\Bigr)^{1/2}
\Bigr\}.
\end{split}
\end{equation}
Here $C_{\mathrm{bias},1},C_{\mathrm{bias},2},C_{\mathrm{var}},C_{\mathrm{proj}}>0$ depend only on $\|\mathbf{K}^{\mathrm{EnDCF}}\|_2$ and $\|\mathbf{R}^{(L)}\|_2$ (and the inverse-stability constant from~\cite{ButlerWildeyZhang2020}), but are independent of $N$. Under Gaussian sampling, the projector-estimation contribution in \eqref{eq:kappa_proxy_objective} scales as $\mathcal{O}(1/N)$.
\end{corollary}

\begin{proof}
The proxy objective \eqref{eq:kappa_proxy_objective} is obtained by collecting the upper bounds in Theorem~\ref{thm:bias_variance} and minimizing the resulting expression over admissible $\kappa$. Under Gaussian sampling,
\(
(\mathbb{E}\|\mathbf{C}_E^{\,s}-\boldsymbol{\Sigma}_E^{\,s}\|_F^4)^{1/2}=\mathcal{O}(1/N)
\)
by Lemma~\ref{lem:cov_frob_fourth_moment}, and finiteness of
\(
\mathbb{E}\|\mathbf{e}^{(1),s}-\boldsymbol{\mu}_E^{\,s}\|^4
\)
yields the stated scaling for the projector-estimation term.
\end{proof}

\begin{remark}[Why an eigenvalue tail does not appear by default]
In Theorem~\ref{thm:bias_variance}, truncation bias is governed by
$\|(\mathbf{I}-\mathbf{P}_\kappa^{\,s})\boldsymbol{\mu}_E^{\,s}\|^2$, that is, by the projection of the \emph{mean innovation} onto the discarded subspace. Eigenvalue-tail quantities such as $\sum_{i>\kappa}\lambda_i^{\,s}$ enter only under additional structural assumptions linking $\boldsymbol{\mu}_E^{\,s}$ to $\boldsymbol{\Sigma}_E^{\,s}$ (or after introducing an additional averaging mechanism).
\end{remark}
...
% ================================================================================
\section{Experimental Framework and Diagnostic Metrics}\label{sec:framework}
...
Four-dimensional methods assimilate windows of $L=5$ consecutive observations, spanning $LT_{\mathrm{obs}}=0.5$ time units. This corresponds to approximately $0.83\,\tau_{\mathrm{Lyap}}$ with $\tau_{\mathrm{Lyap}}\approx0.6$, yielding a temporal autocorrelation of state variables at lag $0.5$ of approximately $0.61$ (computed under the Lorenz-96 invariant measure at $F=8$). We perform $W=50$ windows, for a total of $K=WL=250$ observation times.

\begin{remark}[Metrics evaluation convention]
\label{rem:metrics_convention}
For four-dimensional methods, analysis updates occur once per window (at window endpoints), yielding $W=50$ analysis states. For sequential methods, updates occur at every observation time, yielding $K=WL=250$ analysis states. To enable fair comparison, all performance metrics (spread, RMSE, rank histograms) are computed at \emph{window endpoints only}. We index these evaluation times by $w\in\{1,\ldots,W\}$, corresponding to global observation-time indices $k_w:=wL\in\{L,2L,\ldots,WL\}=\{5,10,\ldots,250\}$. Equations~\eqref{eq:spread_framework}--\eqref{eq:rmse_framework} therefore aggregate over $w=1,\ldots,W$. This convention ensures that (i) 4D methods are evaluated at the times when their analysis is actually computed, and (ii) sequential methods are evaluated at comparable temporal spacing, avoiding artifacts from dense versus sparse evaluation.
\end{remark}

Algorithm-specific parameters are fixed across experiments. QPCA-EnDCF uses truncation level $\kappa=1$ (retaining only the leading eigenmode) and no multiplicative inflation ($\lambda_{\mathrm{infl}}=1.00$), consistent with eigenvalue spectra in which the dominant mode captures $60$--$80\%$ of residual variance. Stochastic methods employ multiplicative inflation $\lambda_{\mathrm{infl}}=1.05$ and perturbed observations drawn from $\mathcal{N}(\mathbf{0},\mathbf{R})$ for sequential updates or $\mathcal{N}(\mathbf{0},\mathbf{R}^{(L)})$ for four-dimensional updates.




% ================================================================================
% SECTION: Probabilistic Calibration Results
% ================================================================================

\section{Probabilistic Calibration Results}\label{sec:calibration}
...
Spread--skill diagnostics assess whether ensemble-estimated uncertainty is consistent with actual estimation error. A calibrated ensemble should satisfy two criteria: its spread should match the RMSE in magnitude (scale consistency) and covary with it over time (dynamic tracking).
...
\end{equation}
where $\widehat{\mathbf{P}}_{wL}^{\,a} = \frac{1}{N-1}\mathbf{A}_{wL}\mathbf{A}_{wL}^{\top}$ is the empirical analysis covariance at the endpoint $k=wL$, formed from ensemble anomalies $\mathbf{A}_{wL} = \mathbf{X}_{wL}^a - \bar{\mathbf{x}}_{wL}^a\mathbf{1}^{\top}$. Estimation accuracy is quantified by the root-mean-square error
\begin{equation}
\mathrm{RMSE}
=
\left[
...
\end{equation}
where $\bar{\mathbf{x}}_{wL}^a$ is the ensemble mean analysis at the endpoint $k=wL$ and $\mathbf{x}_{wL}^{\mathrm{true}}$ is the corresponding true state. Calibration is summarized by the spread--skill ratio
\begin{equation}
\gamma = \frac{\sigma_{\mathrm{ens}}}{\mathrm{RMSE}},
\label{eq:gamma_framework}
\end{equation}
for which ideal calibration corresponds to $\gamma \approx 1$; values $\gamma \ll 1$ and $\gamma \gg 1$ indicate overconfidence and underconfidence, respectively.
...
Table~\ref{tab:calibration_metrics} reports, for each algorithm, trial-level calibration statistics (each computed over $W=50$ windows) summarized as mean $\pm$ standard deviation over five independent trials, including mean ensemble spread, RMSE, average spread--skill ratio $\bar{\gamma}$, and the Pearson correlation $\rho$ between the window-wise spread and RMSE sequences.

\begin{table}[htbp]
\centering
\caption{Probabilistic calibration metrics under the baseline configuration ($N=10$, $L=5$, $\sigma_{\mathrm{obs}}=1.5$). Values are mean $\pm$ standard deviation over five independent seeds, each time-averaged over 50 windows.}
...
\caption{Temporal evolution of ensemble spread (solid) and RMSE (dashed) over the 50-window sequence; shaded regions denote interquartile ranges across five trials. QPCA-EnDCF exhibits coherent covariation between spread and RMSE throughout. Sequential EnKF shows persistently low spread with no tracking of RMSE, while 4D-EnKF displays weak positive correlation but remains underdispersed.}
...
\caption{Probabilistic calibration diagnostics. \textbf{(A)} Spread--skill ratio $\gamma$ across assimilation windows; the dashed line indicates perfect calibration. \textbf{(B)} Reliability diagram plotting ensemble spread against RMSE for all windows and trials, with the diagonal denoting $\sigma_{\mathrm{ens}}=\mathrm{RMSE}$.}
\label{fig:reliability}
\end{figure}

QPCA-EnDCF clusters near the identity line across RMSE values $2$--$5$, confirming accurate uncertainty quantification over varying error regimes. Sequential EnKF exhibits horizontal banding at $\sigma_{\mathrm{ens}}\approx0.3$, consistent with near-zero correlation. The 4D-EnKF shows a weak positive slope but systematic displacement below the diagonal.
...
\label{eq:rank}
\end{equation}
where $x_{k,i}^{(j),a}$ is the $i$th component of analysis ensemble member $j$. Ties (which occur with probability zero under continuous distributions and are empirically rare) are broken randomly. Under perfect calibration, ranks are uniformly distributed on $\{1,\ldots,N+1\}$ with probability $1/(N+1)$. Systematic departures indicate miscalibration: U-shaped histograms reflect overdispersion, dome-shaped histograms indicate underdispersion, and asymmetry signals bias.
...
Consistent with Remark~\ref{rem:metrics_convention}, ranks are computed at window endpoints only. Aggregating across all state components ($n=40$), windows ($W=50$), and trials (5) yields $M_{\mathrm{rank}}=n \times W \times 5 = 10{,}000$ rank samples per algorithm. For $N=10$, there are $N+1=11$ bins, yielding an expected count of $E_b = 10{,}000/11 \approx 909$ per bin.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig04d_rank_histograms.png}
\caption{Rank histogram analysis aggregated over all components, windows, and trials ($10{,}000$ samples per algorithm). The dashed line indicates the uniform frequency $1/11$, and the shaded band denotes the 95\% confidence interval under the uniformity null. Left: QPCA-EnDCF shows near-uniformity ($\chi^2=396.8$, flatness $0.199$). Center: Sequential EnKF exhibits severe dome-shaped distortion ($\chi^2=173{,}195$, flatness $1.861$). Right: 4D-EnKF shows substantial but reduced departure from uniformity ($\chi^2=32{,}269$, flatness $1.796$).}
...
Sequential EnKF is strongly miscalibrated ($\chi^2=173{,}195$, flatness $1.861$), with a pronounced dome shape: central ranks occur far more frequently than expected, while extreme ranks are depleted. This is the canonical signature of severe underdispersion caused by ensemble collapse. The 4D-EnKF shows improvement ($\chi^2=32{,}269$, flatness $1.796$) but retains a dome-shaped structure, indicating persistent underdispersion despite temporal coupling. Increasing the effective dimension from $m=20$ to $mL=100$ without increasing ensemble size exacerbates this limitation.

Overall, the rank histogram analysis demonstrates that QPCA-EnDCF substantially improves distributional calibration beyond variance matching alone. Its flatness metric—an order of magnitude smaller than those of the EnKF variants—indicates that the truth is placed appropriately across the ensemble support. While not perfectly uniform, deviations are minor relative to the severe systematic distortions observed in stochastic methods. The improvement is attributable to the deterministic update and spectral regularization of QPCA-EnDCF, which avoid observation perturbation noise and concentrate corrections into dynamically dominant modes, preserving ensemble diversity and yielding a more faithful approximation of the posterior distribution.
\FloatBarrier
...
The calibration diagnostics in Section~\ref{sec:calibration} established that QPCA-EnDCF yields well-calibrated uncertainty, as measured by spread--skill relationships and rank histograms. These results, however, do not explain \emph{why} calibration improves. We address this question by decomposing the mean squared error (MSE) into bias and variance components. Theory predicts that spectral truncation reduces sampling variance without a commensurate increase in bias, in contrast to classical regularization strategies. Here we provide empirical validation and quantify the relative contributions of bias and variance.
...
In the sequential setting where truth and observations evolve over many assimilation cycles, the decomposition \eqref{eq:mse_decomposition_empirical} applies at each assimilation time (here, at each window endpoint). We therefore estimate bias and variance \emph{as functions of window index} and then time-average over windows to obtain representative values for a method.

For QPCA-EnDCF, theory shows variance scaling as $\mathcal{O}(\kappa/N)$, where $\kappa$ is the truncation level. The bias contribution is governed by $\|(\mathbf{I}-\mathbf{P}_\kappa)\boldsymbol{\mu}_E\|^2$, the component of the mean whitened innovation orthogonal to the retained eigenspace (see Theorem~\ref{thm:bias_variance} and Remark~\ref{rem:truncation_bias_tail}). Importantly, this \emph{cannot} be bounded simply by the discarded spectral tail $\sum_{i>\kappa}\lambda_i$ without additional structure linking $\boldsymbol{\mu}_E$ to $\boldsymbol{\Sigma}_E$ (e.g., a source condition). However, when the mean mismatch is well-aligned with the leading covariance directions—as occurs empirically when the dominant eigenmode captures systematic forecast--observation discrepancy—aggressive truncation ($\kappa=1$) achieves $\mathcal{O}(1/N)$ variance with minimal bias penalty. By contrast, standard ensemble updates exhibit variance scaling as $\mathcal{O}(r/N)$, where $r$ approaches the effective rank of the update.

We estimate bias and variance empirically using $N_{\mathrm{trial}}=10$ independent ensemble realizations per algorithm, differing only in initial ensemble draws. (Note: $N_{\mathrm{trial}}$ denotes the number of Monte Carlo trials, distinct from the ensemble size $N=10$ and the total rank samples $M_{\mathrm{rank}}$ used in Section~\ref{subsec:rank_histograms}.) Let $\bar{\mathbf{x}}_{t,w}^a$ denote the analysis ensemble mean at the end of window $w$ in trial $t$, and let $\mathbf{x}_{w}^{\mathrm{true}}$ denote the corresponding true state (both evaluated at window endpoints). For each window $w$, define the across-trial mean
\[
...
\right\|^2,
\end{align}
with $\widehat{\mathrm{MSE}}_{w}=\widehat{\mathrm{Bias}}_{w}^2+\widehat{\mathrm{Variance}}_{w}$. To summarize performance over the full run, we time-average these window-wise quantities:
\[
...
Figure~\ref{fig:bias_variance_evolution} shows the temporal evolution of the window-wise quantities $\widehat{\mathrm{MSE}}_{w}$, $\widehat{\mathrm{Bias}}_{w}^2$, $\widehat{\mathrm{Variance}}_{w}$, and the ratio $\widehat{\mathrm{Bias}}_{w}^2/\widehat{\mathrm{MSE}}_{w}$ across the 50-window sequence. Curves are computed from $N_{\mathrm{trial}}=10$ independent realizations; shaded bands indicate trial-to-trial variability.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig07a_bias_variance_evolution.png}
\caption{Temporal bias--variance decomposition over 50 assimilation windows. \textbf{(A)} Total MSE. \textbf{(B)} Squared bias. \textbf{(C)} Variance. \textbf{(D)} Bias$^2$/MSE ratio. QPCA-EnDCF achieves substantially lower MSE via dramatic variance reduction, without increased bias.}
\label{fig:bias_variance_evolution}
\end{figure}

QPCA-EnDCF attains MSE $\approx13$, compared to $\approx22$ for Sequential EnKF and $\approx21$ for 4D-EnKF. The decomposition reveals the mechanism: squared bias is comparable across methods (Bias$^2\approx10$), while variance differs sharply. QPCA-EnDCF exhibits variance $\approx2.3$, an $\sim80\%$ reduction relative to stochastic methods (variance $\approx11.6$). Thus, aggressive truncation removes variance-dominated modes while retaining the leading mode capturing systematic mismatch.

The Bias$^2$/MSE ratio further clarifies this distinction. For QPCA-EnDCF, the ratio stabilizes near $82\%$, indicating error dominated by systematic bias rather than sampling noise. In contrast, stochastic methods show ratios near $45$--$47\%$, implying that roughly half of their error arises from irreducible sampling variance at fixed ensemble size.

Figure~\ref{fig:mse_decomposition_bars} summarizes these results via stacked bar charts.
...
QPCA-EnDCF achieves MSE $=12.97$ (Bias$^2=10.67$, Variance$=2.29$), while Sequential EnKF and 4D-EnKF exhibit MSE $\approx21$--$22$ with variance contributions near $11.6$. Normalized bars emphasize the qualitative shift: QPCA-EnDCF error is overwhelmingly systematic, whereas stochastic methods are variance-limited.

These findings validate the theoretical predictions and explain QPCA-EnDCF’s superior calibration. Variance reduction arises from two effects: (i) the deterministic update avoids observation-perturbation noise, whose contribution scales as $C_{\mathrm{pert}}L\,\mathrm{tr}(\mathbf{R})/N$ and is substantial under baseline parameters; and (ii) spectral truncation ($\kappa=1$) concentrates updates in the dominant eigenmode, avoiding estimation of noise-dominated directions. With variance scaling as $\mathcal{O}(\kappa/N)$ rather than $\mathcal{O}(r/N)$, the observed order-of-magnitude reduction follows directly.

Crucially, this variance reduction occurs \emph{without} a bias penalty. Unlike classical penalty-based regularization, which trades variance for increased bias, QPCA-EnDCF maintains bias comparable to or lower than that of stochastic methods while sharply reducing variance. This favorable regime is enabled by rapid spectral decay, whereby leading modes encode signal and higher modes primarily represent sampling noise.

Together with the calibration diagnostics, the bias--variance analysis completes the mechanistic picture. QPCA-EnDCF’s near-ideal spread--skill ratio arises because variance is suppressed without inflating bias, allowing ensemble spread to reflect true error. Stochastic methods suffer from both inflated variance and collapsed spread, yielding severe miscalibration. The dominance of systematic bias in QPCA-EnDCF suggests that further gains are attainable through model or operator improvements, whereas stochastic methods face fundamental variance limits at fixed ensemble size.
\FloatBarrier
...
Covariance inflation counteracts this variance loss by artificially augmenting ensemble spread. Two strategies dominate operational practice.

\emph{Multiplicative inflation} rescales anomalies by a factor $\lambda_{\mathrm{infl}} \ge 1$,
\[
...
uniformly amplifying all eigenvalues while preserving eigenvectors. Geometrically, this is a uniform dilation of the ensemble ellipsoid. Multiplicative inflation is inexpensive and preserves correlation structure, but it cannot address rank deficiency: the null space is unchanged.

\emph{Additive inflation} perturbs each ensemble member with independent noise,
\[
...
We perform a systematic parametric study to quantify inflation requirements across ensemble sizes and filtering methods. Ensemble size is varied over $N \in \{5,10,25,50\}$, multiplicative inflation over $\lambda_{\mathrm{infl}} \in \{1.0,1.05,1.10,1.15,1.20,1.25\}$, and additive inflation over $\alpha_{\mathrm{add}} \in \{0.0,0.01,0.05,0.10,0.15,0.20\}$, with all other parameters fixed at the baseline values of Section~\ref{sec:framework}. For additive inflation we use the standard isotropic choice $\mathbf{Q}=\mathbf{I}_n$. Each configuration is repeated with three independent random seeds. Performance is assessed by time-averaged RMSE over the final 50 assimilation windows, after transients have decayed.
...
\caption{Sensitivity of RMSE to multiplicative inflation. Time-averaged RMSE versus inflation factor $\lambda_{\mathrm{infl}}$ for Sequential EnKF (blue circles), 4D-EnKF (orange squares), and QPCA-EnDCF (green triangles). Panels correspond to $N \in \{5,10,25\}$. Means are averaged over 50 assimilation windows and three trials; shading indicates $\pm1$ standard deviation. QPCA-EnDCF is optimal at $\lambda_{\mathrm{infl}}=1.0$ across all $N$, whereas stochastic methods require $\lambda_{\mathrm{infl}}\approx1.10$ for $N\le10$ and $\lambda_{\mathrm{infl}}\approx1.05$ for larger ensembles.}
\label{fig:mult_inflation}
\end{figure}

QPCA-EnDCF exhibits a uniform response across ensemble sizes: RMSE is minimized at $\lambda_{\mathrm{infl}}=1.0$ (no inflation) and increases monotonically for $\lambda_{\mathrm{infl}}>1.0$. Performance is insensitive to mild inflation ($\lambda_{\mathrm{infl}}\in[1.0,1.05]$) but degrades for larger values, indicating that spectral regularization alone maintains adequate ensemble spread.

By contrast, Sequential EnKF and 4D-EnKF display ensemble-size-dependent optima. For $N=5$, minimum RMSE occurs near $\lambda_{\mathrm{infl}}=1.10$, shifting toward $\lambda_{\mathrm{infl}}=1.05$ as $N$ increases to 25. This trend reflects diminishing finite-sample bias with growing ensemble size. For $N=50$ (omitted from Figure~\ref{fig:mult_inflation} for clarity), the optimal factor is $\lambda_{\mathrm{infl}}\approx1.05$, and the performance gap relative to QPCA-EnDCF narrows to roughly 7--9\%.

The magnitude of this effect is substantial for small ensembles. At $N=5$, Sequential EnKF with optimal inflation ($\lambda_{\mathrm{infl}}=1.10$) attains RMSE $2.15\pm0.18$, compared with $1.68\pm0.12$ for QPCA-EnDCF at $\lambda_{\mathrm{infl}}=1.0$, a reduction of approximately 22\%. Although this advantage decreases with $N$, QPCA-EnDCF retains an $\mathcal{O}(8\%)$ RMSE reduction even at $N=50$.

Figure~\ref{fig:add_inflation} shows the corresponding results for additive inflation with $\mathbf{Q}_{\mathrm{add}}=\mathbf{I}_n$.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_inflation_additive.png}
\caption{Sensitivity of RMSE to additive inflation. Time-averaged RMSE versus additive magnitude $\alpha_{\mathrm{add}}$ for Sequential EnKF, 4D-EnKF, and QPCA-EnDCF, with panels for $N \in \{5,10,25\}$. Plotting conventions match Figure~\ref{fig:mult_inflation}. QPCA-EnDCF is optimal at $\alpha_{\mathrm{add}}=0$ for all $N$. Stochastic methods show mild improvement for $\alpha_{\mathrm{add}}\approx0.05$ when $N\le10$ and $\alpha_{\mathrm{add}}\approx0.01$ for larger ensembles, but remain inferior to multiplicative inflation by 5--10\%.}
\label{fig:add_inflation}
\end{figure}

QPCA-EnDCF again achieves its minimum RMSE at $\alpha_{\mathrm{add}}=0$, with monotonic degradation as $\alpha_{\mathrm{add}}$ increases. Since spectral regularization already controls variance, additive perturbations inject spurious energy that disrupts dynamically consistent correlations without compensating benefits.

For stochastic methods, additive inflation provides limited improvement relative to no inflation, but consistently underperforms multiplicative inflation by 5--10\%. This deficit reflects the difficulty of specifying an appropriate perturbation structure: isotropic noise introduces variance in dynamically irrelevant directions. More structured choices of $\mathbf{Q}_{\mathrm{add}}$ could improve performance but at the cost of additional modeling effort.

Table~\ref{tab:optimal_inflation} summarizes optimal inflation parameters and corresponding RMSE values for all ensemble sizes and methods.

\begin{table}[htbp]
\centering
\caption{Optimal inflation parameters and RMSE. For each method and ensemble size $N \in \{5,10,25,50\}$, we report the multiplicative factor $\lambda_{\mathrm{opt}}$ and additive magnitude $\alpha_{\mathrm{opt}}$ minimizing mean RMSE. Errors are reported as mean $\pm$ standard deviation over three trials, each averaged over 50 assimilation windows.}
\label{tab:optimal_inflation}
\small
\begin{tabular}{llcccc}
\toprule
\textbf{Method} & \textbf{$N$} & \textbf{$\lambda_{\mathrm{opt}}$} & \textbf{$\alpha_{\mathrm{opt}}$} & \textbf{RMSE (mult.)} & \textbf{RMSE (add.)} \\
\midrule
\multirow{4}{*}{Seq-EnKF}
& 5  & 1.10 & 0.05 & $2.15 \pm 0.18$ & $2.28 \pm 0.20$ \\
& 10 & 1.10 & 0.05 & $1.42 \pm 0.09$ & $1.52 \pm 0.10$ \\
& 25 & 1.05 & 0.01 & $1.18 \pm 0.06$ & $1.24 \pm 0.07$ \\
& 50 & 1.05 & 0.01 & $1.11 \pm 0.05$ & $1.15 \pm 0.06$ \\
\midrule
\multirow{4}{*}{4D-EnKF}
& 5  & 1.10 & 0.05 & $2.03 \pm 0.16$ & $2.19 \pm 0.18$ \\
& 10 & 1.10 & 0.05 & $1.38 \pm 0.08$ & $1.47 \pm 0.09$ \\
& 25 & 1.05 & 0.01 & $1.15 \pm 0.05$ & $1.20 \pm 0.06$ \\
& 50 & 1.05 & 0.01 & $1.09 \pm 0.04$ & $1.13 \pm 0.05$ \\
\midrule
\multirow{4}{*}{QPCA-EnDCF}
& 5  & 1.00 & 0.00 & $1.68 \pm 0.12$ & $1.68 \pm 0.12$ \\
& 10 & 1.00 & 0.00 & $1.23 \pm 0.05$ & $1.25 \pm 0.06$ \\
& 25 & 1.00 & 0.00 & $1.08 \pm 0.04$ & $1.08 \pm 0.04$ \\
& 50 & 1.00 & 0.00 & $1.02 \pm 0.03$ & $1.02 \pm 0.03$ \\
\bottomrule
\end{tabular}
\end{table}

The defining feature of Table~\ref{tab:optimal_inflation} is the invariance of QPCA-EnDCF's optimal parameters: $(\lambda_{\mathrm{opt}},\alpha_{\mathrm{opt}})=(1.0,0.0)$ for all $N$. This eliminates the need for manual tuning or adaptive inflation schemes. In contrast, stochastic methods require ensemble-size-dependent inflation, with $\lambda_{\mathrm{opt}}$ decreasing toward unity as $N$ increases. The persistent inferiority of additive inflation relative to multiplicative inflation across all methods and ensemble sizes (visible in the consistently higher RMSE (add.) values) confirms that variance control through ensemble-wide rescaling is more effective than introducing isotropic random perturbations, which lack dynamical structure and contaminate correlations.
...
Previous sections evaluate performance under idealized observational assumptions: Gaussian errors with known diagonal covariance. These assumptions simplify analysis and isolate algorithmic effects, but they are routinely violated in practice. Real observing systems may exhibit outliers (heavy tails), saturation or retrieval nonlinearities (skewness), and shared contamination or systematic biases (spatial correlations), all of which undermine the classical modeling assumptions. The operational question is whether QPCA-EnDCF’s gains persist under such non-idealities, or whether they are specific to the Gaussian/diagonal setting.

We probe robustness by varying observation error characteristics along two axes that isolate distinct failure modes. Section~\ref{subsec:nongaussian} studies \emph{distributional} departures by testing nine error laws spanning symmetric heavy-tailed, right-skewed, and Gaussian cases while keeping the second-moment structure correctly specified. Section~\ref{subsec:correlated_errors} studies \emph{structural} departures by introducing spatial correlations with condition numbers spanning five orders of magnitude, again under correctly specified covariances. Together, these experiments distinguish sensitivity to higher-order distributional features (skewness, kurtosis, tail behavior) from sensitivity to correlation structure.

Unless otherwise stated, all experiments use the baseline configuration of Section~\ref{sec:framework} ($N=10$, $L=5$, $n=40$), modifying only the targeted observation model component.

% ================================================================================
\subsection{Non-Gaussian Observation Error Distributions}
\label{subsec:nongaussian}

Observation errors often depart from Gaussianity due to outliers, saturation, and nonlinear retrieval maps. Although Kalman-type derivations assume Gaussian noise, ensemble methods are frequently argued to be more robust because uncertainty is represented by samples rather than a parametric likelihood.

Here we test whether the performance advantages observed under Gaussian errors persist under non-Gaussian noise. The design isolates \emph{shape} effects from covariance misspecification: every distribution is standardized to mean zero and variance $\sigma_{\mathrm{obs}}^2=2.25$, and all methods are supplied the correct covariance $\mathbf{R}=2.25\mathbf{I}_m$. Thus any performance changes reflect higher-order properties rather than incorrect second-moment information.

We consider nine distributions. The Gaussian $\mathcal{N}(0,\sigma_{\mathrm{obs}}^2)$ serves as a reference. Symmetric heavy-tailed cases include Student-$t$ with $\nu\in\{3,5,10\}$ degrees of freedom and Laplace with scale $b=\sigma_{\mathrm{obs}}/\sqrt{2}$, each centered at zero and rescaled to variance $\sigma_{\mathrm{obs}}^2$. Right-skewed cases include Gamma($k,\theta$) with shape $k\in\{2,5\}$ and Log-normal($\mu,\sigma_{\mathrm{LN}}$) with parameters $\sigma_{\mathrm{LN}}\in\{0.5,0.8\}$. These positive-valued distributions are shifted (by subtracting their mean) and rescaled to achieve mean zero and variance $\sigma_{\mathrm{obs}}^2$ while preserving their characteristic tail behavior and asymmetry. Thus, we use shifted and rescaled Gamma/Log-normal errors rather than the original positive-valued distributions.
...
\caption{Representative non-Gaussian observation error distributions used in the robustness study (Gaussian reference shown as a red dashed curve). Student-$t$ and Laplace errors are symmetric and heavy-tailed; Gamma and Log-normal errors are right-skewed with increasingly heavy upper tails. All distributions are standardized to mean zero and variance $\sigma_{\mathrm{obs}}^2=2.25$, so comparisons isolate sensitivity to distributional shape.}
\label{fig:noise_distributions}
\end{figure}

For each distribution we run three independent trials, each consisting of 10 initialization windows followed by 50 assimilation windows, yielding 150 window-level samples per method--distribution pair. Table~\ref{tab:nongaussian_performance} reports mean RMSE (trial-averaged over the 50 windows) and the relative improvement of QPCA-EnDCF compared with the stochastic methods.
...
QPCA-EnDCF is essentially insensitive to distributional shape: RMSE remains in the narrow band 3.51--3.69 across all nine laws, from extreme heavy tails (Student-$t$, $\nu=3$) to strong right skew (Log-normal, $\sigma_{\mathrm{LN}}=0.8$). The cross-distribution standard deviation is $0.052$ (coefficient of variation $\approx 1.4\%$), indicating near-constant accuracy despite substantial changes in skewness and kurtosis. In contrast, the stochastic methods are both less accurate and more variable across distributions (cross-distribution standard deviations $0.092$ for Sequential EnKF and $0.131$ for 4D-EnKF).

Across all distributions, QPCA-EnDCF improves RMSE by 18.3--24.5\% relative to Sequential EnKF (mean 20.4\%) and by 17.3--22.7\% relative to 4D-EnKF (mean 19.8\%). Figure~\ref{fig:mean_rmse_comparison} summarizes these gains and highlights the comparatively tight spread of QPCA-EnDCF across distributional families.
...
\caption{RMSE evolution over 50 assimilation windows (after 10-window initialization) under each non-Gaussian error law (curves: mean; shading: 95\% confidence intervals across $n=3$ trials). QPCA-EnDCF maintains consistently lower RMSE than both stochastic methods across all distributions and time steps.}
\label{fig:rmse_trajectories}
\end{figure}

These results are consistent with the fact that QPCA-EnDCF is driven by second-moment structure. The whitening transform depends only on $\mathbf{R}$, which is identical and correctly specified in all experiments, and the subsequent spectral truncation is performed on an empirical covariance of whitened residuals. In contrast, stochastic EnKF variants rely on sampling perturbed observations from an assumed noise law, so changes in tail behavior or asymmetry can propagate through the update and increase variability. The deterministic, covariance-based update in QPCA-EnDCF avoids this mechanism, yielding the observed distributional robustness.
...
This subsection tests whether QPCA-EnDCF's whitening step provides robustness to correlated observation errors, and whether its advantages persist as correlation strength and conditioning increase. Importantly, we avoid substantial covariance misspecification: in every experiment, the filter is provided a covariance matrix $\mathbf{R}$ that matches the true error correlation structure. For severely ill-conditioned cases, a small diagonal regularization is applied to ensure numerical stability (see below), which represents a minor departure from exact specification rather than a modeling assumption. The question is therefore algorithmic: \emph{given known correlations, does whitening effectively decorrelate the errors and recover performance close to the diagonal baseline?}

For non-diagonal $\mathbf{R}$, whitening uses
\[
\mathbf{W} = (\mathbf{R}^{(L)})^{-1/2},
\qquad
\mathbf{W}\mathbf{R}^{(L)}\mathbf{W}^{\top} = \mathbf{I},
\]
a bijective linear change of coordinates that standardizes and decorrelates the observation errors. We consider two standard implementations: (i) Cholesky whitening, $\mathbf{R}=\mathbf{L}\mathbf{L}^{\top}$ with $\mathbf{W}=\mathbf{L}^{-\top}$, and (ii) symmetric square-root whitening, $\mathbf{R}=\mathbf{U}\boldsymbol{\Lambda}\mathbf{U}^{\top}$ with $\mathbf{W}=\mathbf{U}\boldsymbol{\Lambda}^{-1/2}\mathbf{U}^{\top}$. Both have $\mathcal{O}(m^3)$ cost; the comparison here is numerical behavior under severe ill-conditioning.

We test three covariance structures spanning five orders of magnitude in condition number. The diagonal baseline $\mathbf{R}=\sigma_{\mathrm{obs}}^2\mathbf{I}_m$ with $\sigma_{\mathrm{obs}}=1.5$ has $\mathrm{cond}(\mathbf{R})=1$. The exponential correlation model
\[
...
\caption{Observation error covariance structures used in the correlation robustness study. Left: diagonal baseline ($\mathrm{cond}(\mathbf{R})=1$). Middle: exponential correlation with $\ell_{\mathrm{corr}}=4$ ($\mathrm{cond}(\mathbf{R})=649$). Right: Gaussian correlation with $\ell_{\mathrm{corr}}=4$ ($\mathrm{cond}(\mathbf{R})\approx 3.7\times10^5$), exhibiting strong near-diagonal correlations and severe ill-conditioning. Periodic distances are used to avoid boundary effects.}
\label{fig:correlation_structures}
\end{figure}

For each structure we compare QPCA-EnDCF with Cholesky and symmetric square-root whitening against 4D-EnKF as a stochastic baseline. All runs use a single realization with 50 assimilation windows following 10-window initialization. This choice reflects the substantial cost of repeated experiments under severely ill-conditioned covariances; the long window sequence still provides a meaningful within-run comparison.

Table~\ref{tab:correlated_obs_summary} reports mean RMSE over the 50-window sequence, along with (i) degradation relative to the diagonal case for each method and (ii) QPCA-EnDCF’s improvement relative to 4D-EnKF under the same correlation structure.

\begin{table}[htbp]
\centering
\caption{Reconstruction error under correlated observation errors. Values are mean RMSE over 50 assimilation windows (after 10-window initialization) from a single realization. Column ``vs. Diag'' reports percent change relative to the diagonal baseline for the same method. Column ``vs. 4D'' reports percent improvement of QPCA-EnDCF relative to 4D-EnKF under the same correlation structure (negative values indicate improvement).}
\label{tab:correlated_obs_summary}
\small
\begin{tabular}{llcccc}
\toprule
\textbf{Correlation} & \textbf{Method} & \textbf{Cond($\mathbf{R}$)} & \textbf{RMSE} & \textbf{vs. Diag} & \textbf{vs. 4D} \\
\midrule
\multirow{3}{*}{Diagonal (baseline)}
    & QPCA-Cholesky & $1.00$ & $3.47$ & --- & $-25.2\%$ \\
    & QPCA-SymSqrt & $1.00$ & $3.47$ & $0.0\%$ & $-25.2\%$ \\
    & 4D-EnKF & $1.00$ & $4.64$ & --- & --- \\
\midrule
\multirow{3}{*}{Exponential ($\ell=4$)}
    & QPCA-Cholesky & $649$ & $3.71$ & $+6.9\%$ & $-24.7\%$ \\
    & QPCA-SymSqrt & $649$ & $3.71$ & $+6.9\%$ & $-24.7\%$ \\
    & 4D-EnKF & $649$ & $4.93$ & $+6.2\%$ & --- \\
\midrule
\multirow{3}{*}{Gaussian ($\ell=4$)}
    & QPCA-Cholesky & $3.7 \times 10^5$ & $3.65$ & $+5.1\%$ & $-31.9\%$ \\
    & QPCA-SymSqrt & $3.7 \times 10^5$ & $3.65$ & $+5.1\%$ & $-31.9\%$ \\
    & 4D-EnKF & $3.7 \times 10^5$ & $5.35$ & $+15.3\%$ & --- \\
\bottomrule
\end{tabular}
\end{table}

QPCA-EnDCF is robust across condition numbers spanning five orders of magnitude. Relative to the diagonal baseline, RMSE increases only modestly under correlation: from 3.47 (diagonal) to 3.71 (exponential, $+6.9\%$) and 3.65 (Gaussian, $+5.1\%$). Even under $\kappa(\mathbf{R})\approx 3.7\times10^5$, performance remains within 7\% of the uncorrelated reference, indicating that whitening effectively neutralizes the impact of correlation on the subsequent spectral filtering.

The two whitening implementations are numerically indistinguishable at the level relevant for filtering: both yield the same RMSE to two decimal places in every case (maximum difference $<0.005$). Given this equivalence and the favorable constant factors of Cholesky, Cholesky whitening is the natural default when $\mathbf{R}$ is positive definite.

By contrast, 4D-EnKF exhibits substantially larger degradation as correlations strengthen, particularly in the severely ill-conditioned Gaussian case (RMSE 4.64 $\to$ 5.35, $+15.3\%$). Consequently, QPCA-EnDCF's advantage \emph{increases} with correlation strength: the reduction relative to 4D-EnKF is 25.2\% (diagonal), 24.7\% (exponential), and 31.9\% (Gaussian). Thus the whitening-based deterministic update is most beneficial precisely in regimes where correlations reduce the effective independent information content and stochastic perturbations become less efficient.
...
Stochastic filters further introduce variance from observation perturbations, scaling like $\mathcal{O}(\mathrm{tr}(\mathbf{R})/N)$ per update. QPCA-EnDCF eliminates this perturbation component; the following experiments quantify the practical impact.

We vary $N \in \{5,10,15,20,30,50,100\}$ while holding the baseline configuration fixed ($n=40$, $m=20$, $L=5$, $\sigma_{\mathrm{obs}}=1.5$). This spans extreme undersampling ($N=5$, rank-$4$ covariances, $N/n=0.125$) through moderate undersampling ($N=20$, $N/n=0.5$) to well-sampled conditions ($N=100$, $N/n=2.5$). Each setting uses three independent trials with 25-window initialization followed by 50 assimilation windows.

Figure~\ref{fig:performance_degradation} summarizes reconstruction accuracy. Panel (a) reports absolute RMSE versus $N$; panel (b) reports degradation relative to the $N=100$ baseline.
...
Table~\ref{tab:ensemble_size_comparison_operational} reports RMSE values with 95\% confidence intervals and the associated improvements of QPCA-EnDCF over the stochastic baselines.

\begin{table}[htbp]
\centering
\caption{Reconstruction error versus ensemble size. Values are mean RMSE $\pm$ 95\% confidence interval over three trials, each averaged over 50 assimilation windows. ``Improvement'' reports the percentage RMSE reduction of QPCA-EnDCF relative to Sequential EnKF and 4D-EnKF; $p$-values are from paired $t$-tests on window-level RMSE across trials.}
\label{tab:ensemble_size_comparison_operational}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Size} & \textbf{Seq-EnKF} & \textbf{4D-EnKF} & \textbf{QPCA-EnDCF} & \textbf{Improvement} & \textbf{$p$-value} \\
\midrule
$N=5$   & $4.71 \pm 0.05$ & $4.86 \pm 0.03$ & $4.27 \pm 0.03$ & 9.3--12.1\%   & $p < 0.05$ \\
$N=10$  & $4.55 \pm 0.11$ & $4.46 \pm 0.08$ & $3.70 \pm 0.04$ & 17.0--18.7\% & $p < 0.05$ \\
$N=15$  & $4.28 \pm 0.09$ & $4.14 \pm 0.02$ & $3.54 \pm 0.01$ & 14.5--17.3\% & $p < 0.05$ \\
$N=20$  & $3.99 \pm 0.07$ & $3.79 \pm 0.05$ & $3.44 \pm 0.02$ & 9.2--13.8\%  & $p < 0.01$ \\
$N=30$  & $3.62 \pm 0.04$ & $3.55 \pm 0.04$ & $3.40 \pm 0.01$ & 4.2--6.1\%   & $p < 0.05$ \\
$N=50$  & $3.45 \pm 0.02$ & $3.42 \pm 0.02$ & $3.36 \pm 0.01$ & 1.8--2.6\% & $p < 0.05$ \\
$N=100$ & $3.33 \pm 0.01$ & $3.33 \pm 0.01$ & $3.33 \pm 0.01$ & $\sim 0\%$ & N.S. \\
\bottomrule
\end{tabular}
\end{table}

Two operational patterns emerge. First, QPCA-EnDCF remains viable even under extreme undersampling: at $N=5$ it achieves RMSE $4.27\pm0.03$, improving on both stochastic methods. Second, the gap narrows with increasing $N$ as sampling error decreases for all methods; performance becomes statistically indistinguishable by $N=100$.

The scaling also translates directly into computational savings. QPCA-EnDCF at $N=10$ (RMSE $3.70\pm0.04$) is comparable to Sequential EnKF at $N=30$ (RMSE $3.62\pm0.04$) and improves on 4D-EnKF at $N=20$ (RMSE $3.79\pm0.05$). Thus, for a fixed accuracy target, QPCA-EnDCF can reduce ensemble size by roughly $2$--$3\times$, cutting forecast propagation cost by 50--67\%.

Reconstruction accuracy alone is not sufficient for deployment; reliable uncertainty quantification is equally important. We therefore extend the calibration diagnostics of Section~\ref{sec:calibration} across $N \in \{5,10,15,20,30,50\}$ using spread--skill ratio $\bar{\gamma}$ (averaged over five trials and 50 windows per trial). Figure~\ref{fig:ensemble_calibration} reports $\bar{\gamma}$ versus $N$; the target $\bar{\gamma}=1$ indicates calibrated spread.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_ensemble_size_spread_skill.png}
\caption{Spread--skill ratio $\bar{\gamma}$ versus ensemble size $N \in \{5,10,15,20,30,50\}$. Points show means over five trials; error bars denote $\pm1$ standard deviation. The dashed line marks the ideal target $\bar{\gamma}=1$. QPCA-EnDCF improves monotonically with $N$ (from 0.38 at $N=5$ to 0.95 at $N=50$), while Sequential EnKF and 4D-EnKF remain strongly underdispersed across all $N$ (below 0.15 even at $N=50$).}
...
Window length $L$ controls how many observation times are assimilated jointly, and therefore trades information content against computational and modeling costs. As $L$ grows, the number of assimilated observations increases linearly ($mL$), which can improve estimation by exploiting temporal constraints. The costs are also clear: matrix sizes grow from $m\times m$ at $L=1$ to $mL\times mL$ for $L>1$, with dense linear algebra scaling as $\mathcal{O}(m^3L^3)$. Longer windows also propagate forecasts further, increasing susceptibility to accumulated model error, and—once $L$ exceeds the decorrelation time—additional observations become largely redundant, yielding diminishing returns.

For QPCA-EnDCF, $L$ directly changes the dimension of the whitened residual covariance $\mathbf{C}_E\in\mathbb{R}^{mL\times mL}$. Increasing $L$ provides a richer temporal residual structure from which principal modes can be identified, but gains should saturate beyond the decorrelation scale and may degrade if extra observations primarily contribute noise. In stochastic 4D-EnKF, longer windows can help through improved conditioning, but the impact is tempered by the accumulation of perturbation noise across multiple observation times.

We vary $L\in\{1,3,5,7,10,15\}$ and adjust the number of windows $W$ so that the total number of assimilated observations satisfies $LW\approx 400$. This keeps the overall information budget approximately fixed, so differences reflect algorithmic response to temporal coupling rather than increased sample size. Each configuration uses three independent trials.

Figure~\ref{fig:rmse_vs_L} reports reconstruction accuracy and the relative advantage of QPCA-EnDCF as $L$ varies. Panel (a) shows RMSE versus $L$ for all methods; panel (b) shows the percent improvement of QPCA-EnDCF relative to 4D-EnKF.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/combined_window_rmse_analysis.png}
\caption{Window-length sensitivity for $L\in\{1,3,5,7,10,15\}$ with $LW\approx 400$ held approximately constant. Error bars denote $\pm1$ standard deviation across three trials. \textbf{(a)} RMSE versus $L$ for Sequential EnKF (blue), 4D-EnKF (green), and QPCA-EnDCF (red). QPCA-EnDCF improves sharply from $L=1$ to $L=3$ and then plateaus for $L\ge 5$. \textbf{(b)} Percent improvement of QPCA-EnDCF relative to 4D-EnKF versus $L$. QPCA-EnDCF underperforms slightly at $L=1$ but achieves substantial improvements for all $L\ge 3$.}
\label{fig:rmse_vs_L}
\end{figure}
...
For $L\ge 5$, QPCA-EnDCF stabilizes (RMSE $\approx 3.64$--$3.68$ for $L\in\{5,7,10\}$) and increases only slightly at $L=15$. This plateau is consistent with diminishing returns once the window exceeds the temporal decorrelation scale: additional observations contribute limited independent information, while larger windows increase cost and may amplify model error or introduce weak spurious modes.

Stochastic methods are far less sensitive to $L$. Sequential EnKF is essentially flat across the tested range (RMSE 4.58--4.72). 4D-EnKF improves modestly from $4.64\pm0.19$ at $L=1$ to $4.40\pm0.13$ at $L=15$ (5.1\%), suggesting a limited benefit from temporal coupling that is small relative to QPCA-EnDCF’s $L=1\to3$ improvement.

Panel (b) emphasizes the regime change: QPCA-EnDCF is slightly worse than 4D-EnKF at $L=1$ (improvement $-5.1\%$), but for every $L\ge 3$ it outperforms 4D-EnKF by 16.3--21.0\%, with the largest gain at $L=3$. Thus, temporal coupling is essential for QPCA-EnDCF: once $L$ is large enough to expose coherent cross-time residual structure, spectral regularization delivers a substantial and stable advantage.

Operationally, these results suggest (i) QPCA-EnDCF should be used with $L\ge 3$, and (ii) $L\in[5,10]$ provides a good balance between accuracy and cost. Conversely, if strict sequential processing is required ($L=1$), 4D-EnKF is competitive and may be preferable.
...
The preceding analyses established QPCA-EnDCF's advantages across theoretical, experimental, and operational evaluation dimensions. Theoretical development (Section~\ref{sec:problem}) characterized variance preservation under spectral projection and bias-variance tradeoffs governing calibration quality. Controlled experiments (Sections~\ref{sec:calibration}--\ref{sec:robustness}) demonstrated superior reconstruction accuracy, near-ideal probabilistic calibration without inflation, and robustness to observational non-idealities. Operational studies (Section~\ref{sec:operational}) quantified performance persistence from extreme undersampling through well-sampled regimes and identified window length requirements for effective operation.

These results challenge conventional assumptions that stochastic observation perturbations are necessary for calibrated uncertainty quantification. The demonstration that deterministic spectral projection yields superior calibration suggests perturbation noise represents a fundamental limitation rather than a correctable artifact. The theoretical framework reveals the mechanism: spectral regularization confines corrections to low-rank signal subspaces while preserving variance exactly in orthogonal directions, implementing variance conservation with sampling efficiency scaling as retained rank rather than observation dimension.

We synthesize theoretical and empirical findings, examine methodological limitations, and situate results within broader ensemble filtering literature. Section~\ref{subsec:discussion_theory} develops geometric interpretations of variance preservation. Section~\ref{subsec:discussion_limitations} identifies experimental idealizations constraining generalizability. Section~\ref{subsec:discussion_broader} connects spectral regularization to inverse problem theory and alternative ensemble transform approaches.
% ================================================================================

\subsection{Theoretical Insights}\label{subsec:discussion_theory}

The variance preservation mechanism admits geometric interpretation. Ensemble data assimilation constrains uncertainty along directions aligned with observation sensitivity while ideally preserving uncertainty in orthogonal directions where observations provide no information. QPCA-EnDCF implements explicit orthogonal projection: corrections lie in the subspace spanned by dominant eigenmodes of whitened residuals, while ensemble diversity is preserved exactly in the orthogonal complement. This contrasts with stochastic methods where finite sampling causes perturbations to correlate spuriously with Kalman gain directions, inducing compression along intended preservation directions.

The bias-variance decomposition reveals that sampling variance scales as $\kappa/N$ (number of retained modes over ensemble size) rather than $mL/N$ (observation dimension over ensemble size). For $\kappa = 1$ and $mL = 100$, this represents theoretical order-of-magnitude sampling efficiency advantage. Truncation bias can be small when forecast errors concentrate in leading eigenmodes, a condition satisfied for chaotic systems near unstable manifolds. Empirical eigenvalue analyses confirm substantial concentration with the leading eigenvalue capturing 60--80\% of total residual variance (Section~\ref{subsec:benchmark}).

These findings suggest deterministic filtering through spectral projection may provide more faithful discrete approximation to Bayesian inference under severe undersampling than stochastic sampling approaches when posterior uncertainty concentrates in low-rank subspaces.
% ================================================================================

\subsection{Methodological Limitations}\label{subsec:discussion_limitations}

Several idealizations constrain generalizability. Our experiments employ identical models for truth generation and forecasts, eliminating structural model error. Operational systems inevitably confront systematic forecast biases from parameterization deficiencies or missing physics. Structural error may alter comparative performance in ways our experiments cannot assess. Spectral filtering's aggressive regularization might benefit from model error by suppressing spurious corrections, or prove disadvantageous if systematic biases require additional spread. Future validation should evaluate performance under controlled model error.

We employ fixed spectral truncation at $\kappa = 1$ based on information criteria. While optimal for Lorenz-96 in the severely undersampled regime, complex systems with multiscale structure might require $\kappa = 2$--$5$ modes. Adaptive online selection based on information criteria evaluated at each window could improve robustness with modest computational overhead.

Experimental design employs several observational idealizations: synchronous observations via perfect linear operators, homogeneous error variance, and simple correlation structures. Operational networks exhibit asynchronous arrival requiring temporal interpolation, nonlinear observation operators, and precision variations spanning orders of magnitude. The Lorenz-96 state dimension $n = 40$ falls far below operational scales ($n \sim 10^6$--$10^9$). Validation on intermediate-complexity testbeds would establish scaling behavior toward operational systems.

Baseline ensemble size $N = 10$ emphasizes severely undersampled regimes. While demonstrating advantages under these conditions, operational systems typically employ $N \sim 20$--$100$. Relative accuracy advantages observed at $N = 10$ may narrow at larger ensemble sizes, though calibration advantages appear to persist based on scaling experiments to $N = 50$.
% ================================================================================

\subsection{Connections to Related Work}\label{subsec:discussion_broader}

From inverse problem theory perspective, QPCA-EnDCF implements data-adaptive Tikhonov regularization through spectral truncation. The critical distinction from classical truncated singular value decomposition is adaptive regularization: eigenvectors are recomputed at each window from observed residual structure, providing automatic regularization strength adjustment to forecast quality without explicit hyperparameter specification.

QPCA-EnDCF shares conceptual similarities with ensemble transform Kalman filters, which also operate in low-dimensional subspaces and avoid observation perturbations. Key differences include transformation domain (observation space versus ensemble space), rank reduction mechanism (aggressive spectral truncation versus full ensemble rank retention), and temporal coupling requirements (windowing essential versus sequential operation).

More broadly, our findings contribute to computational strategies for high-dimensional Bayesian inference under severe undersampling. Results suggest that variational approaches with aggressive regularization can provide well-calibrated uncertainty quantification compared to sampling approaches when posterior distributions concentrate in low-rank subspaces, ensemble sizes satisfy $N \ll n$, and computational budgets limit ensemble sizes to regimes where sampling errors dominate. This may extend to other high-dimensional inference contexts where similar geometric concentration and severe undersampling occur.
...
This work developed a theoretical framework for deterministic spectral regularization in ensemble data assimilation and demonstrated through controlled experiments that such methods achieve superior probabilistic calibration compared to conventional stochastic approaches. Three theoretical results characterize the mechanism: (i) deterministic spectral projection avoids variance injection from stochastic observation perturbations and leaves residual components in noise-dominated orthogonal directions unmodified; (ii) decomposition of estimation error reveals sampling variance scales with retained spectral rank ($\kappa$) rather than observation dimension ($mL$), conferring order-of-magnitude sampling efficiency under aggressive regularization; (iii) asymptotic analysis establishes near-ideal calibration as ensemble size increases, demonstrating deterministic projection can approximate Bayesian inference when posterior uncertainty concentrates in low-rank subspaces.

Numerical experiments on the Lorenz-96 system confirm theoretical predictions. QPCA-EnDCF achieves spread-skill ratios approaching unity ($\bar{\gamma} = 0.811\pm0.101$ at the baseline $N=10$) while stochastic methods remain severely underdispersed ($\bar{\gamma} \approx 0.10$--$0.12$) despite covariance inflation. The method requires no inflation, exhibits robustness to non-Gaussian errors and spatial correlations, and maintains advantages from extreme undersampling ($N = 5$) through well-sampled regimes ($N = 50$). Geometric interpretation reveals the mechanism: spectral projection confines corrections to low-rank signal subspaces while preserving ensemble diversity exactly in orthogonal directions. This contrasts with stochastic perturbations where finite sampling induces spurious correlations, causing variance compression along intended preservation directions.

Operational studies establish practical deployment guidelines. QPCA-EnDCF at $N = 10$ achieves accuracy comparable to stochastic methods at $N = 20$--$30$ and calibration superior to stochastic methods at $N = 50$, enabling 2--5$\times$ computational savings. Window length analysis reveals sharp transitions: sequential operation ($L = 1$) proves ineffective, but windowed operation with $L \geq 3$ yields 20+ percentage point accuracy advantages. These findings establish minimum viable operating points and quantify computational savings in resource-constrained settings.

Several idealizations constrain generalizability. Perfect-model experiments eliminate structural error pervasive in operational systems. Observational configurations remain idealized compared to operational networks with asynchronous arrival, nonlinear operators, and heterogeneous precision. State dimension ($n = 40$) falls far below operational scales ($n \sim 10^6$--$10^9$). Priority extensions include validation under model error, adaptive rank selection, intermediate-complexity testbeds, and operational-scale validation. Whether favorable properties demonstrated in idealized chaotic systems persist at operational scales and extend to other high-dimensional inference contexts remains an open question warranting investigation.
...
\For{$k = 1$ to $K$}
    ...
    \EndFor
    \State $\bar{\mathbf{x}}^f \gets \frac{1}{N}\sum_{j=1}^{N}\mathbf{x}^{(j)}$, \quad $\mathbf{A}^f \gets \lambda_{\mathrm{infl}}(\mathbf{X} - \bar{\mathbf{x}}^f\mathbf{1}^{\top})$ \Comment{Inflated anomalies}
...
Observation perturbations $\boldsymbol{\epsilon}_{k}^{(j)} \sim \mathcal{N}(\mathbf{0}, \mathbf{R})$ are introduced to preserve ensemble variance in expectation. Multiplicative inflation $\lambda_{\mathrm{infl}} > 1$ provides additional variance augmentation.
% ================================================================================
...
\For{$w = 1$ to $W$}
    ...
    \State $\mathbf{A}_x \gets \lambda_{\mathrm{infl}}(\mathbf{X}_L - \bar{\mathbf{x}}_L^f\mathbf{1}^{\top})$, \quad $\mathbf{A}_y \gets \mathbf{Y}_{\mathrm{stack}} - \bar{\mathbf{y}}_{\mathrm{stack}}\mathbf{1}^{\top}$
    \State $\mathbf{P}_{xy} \gets \frac{1}{N-1}\mathbf{A}_x\mathbf{A}_y^{\top}$, \quad $\mathbf{P}_{yy} \gets \frac{1}{N-1}\mathbf{A}_y\mathbf{A}_y^{\top}$
    \State $\mathbf{K}^{(w)} \gets \mathbf{P}_{xy}(\mathbf{P}_{yy} + \mathbf{R}^{(L)})^{-1}$
    \For{$j = 1$ to $N$}
...
Joint processing of $L$ observations exploits temporal correlations through extended cross-covariance $\mathbf{P}_{xy} \in \mathbb{R}^{n \times mL}$. Perturbations are sampled from $\mathcal{N}(\mathbf{0}, \mathbf{R}^{(L)})$ with block-diagonal covariance structure.
% ================================================================================
...
\For{$w = 1$ to $W$}
    ...
    \State
	    ...
	    \State $[\mathbf{V}, \boldsymbol{\Lambda}] \gets \mathrm{eig}(\mathbf{C}_E)$ \Comment{$\mathbf{V}=[\mathbf{v}_1,\ldots,\mathbf{v}_d]$, $\boldsymbol{\Lambda}=\mathrm{diag}(\lambda_1,\ldots,\lambda_d)$}
    ...
    \State \textit{// Update}
	    ...
	    \State $\mathbf{A}_y \gets \mathbf{Y}_{\mathrm{stack}} - \bar{\mathbf{y}}_{\mathrm{stack}}\mathbf{1}^{\top}$
	    \State $\mathbf{P}_{xy} \gets \frac{1}{N-1}\mathbf{A}_x\mathbf{A}_y^{\top}$, \quad $\mathbf{P}_{yy} \gets \frac{1}{N-1}\mathbf{A}_y\mathbf{A}_y^{\top}$
	    \State $\mathbf{K}^{\mathrm{EnDCF}} \gets \mathbf{P}_{xy}(\mathbf{P}_{yy})^{\dagger}$
	    \State $\mathbf{X}_L \gets \mathbf{X}_L + \mathbf{K}^{\mathrm{EnDCF}}\boldsymbol{\Delta}_{\mathrm{obs}}$
    \State $\mathbf{X} \gets \mathbf{X}_L$
\EndFor
\State \Return $\mathbf{X}$
\end{algorithmic}
...
The whitening transformation (line 12) normalizes residuals so observation errors have identity covariance in transformed space. The algorithm accepts general $\mathbf{R}$. For the diagonal special case $\mathbf{R}=\sigma_{\mathrm{obs}}^2\mathbf{I}_m$, this reduces to scalar division by $\sigma_{\mathrm{obs}}$ and can be implemented efficiently. For correlated errors, matrix square root requires Cholesky decomposition or symmetric eigendecomposition.

Spectral truncation (line 19) implements hard thresholding: $\mathbf{Q}_{\mathrm{PCA}} = -\hat{\mathbf{V}}_{\kappa}\hat{\mathbf{V}}_{\kappa}^{\top}\mathbf{E}$ retains only leading $\kappa$ eigenmodes. The negative sign ensures corrections shift ensemble toward observations.

The algorithm applies a deterministic correction to the ensemble without stochastic observation perturbations (line 25). As written, $\boldsymbol{\Delta}_{\mathrm{obs}}$ is a matrix of memberwise observation-space increments derived from the residual matrix, so corrections are deterministic but generally member dependent. State anomalies are computed without inflation factor (line 23), in contrast to explicit inflation in stochastic methods.
...
% % We distinguish population quantities (deterministic objects defined from the forecast law) from their \emph{sample}
...
% 	$\mathbf{K}_k$ & $\mathbb{R}^{n\times m}$ & Sequential Kalman gain at time $k$ (computed using sample covariance $\widehat{\mathbf{P}}^{\,f}$ in practice) \\
% 	$\mathbf{K}^{(w)}$ & $\mathbb{R}^{n\times d}$ & Windowed Kalman gain for window $w$ (with $d=mL$, computed from sample covariances) \\
% $\mathbf{K}^{\mathrm{EnDCF}}$ & $\mathbb{R}^{n\times d}$ & \textbf{Empirical} EnDCF gain (random), defined via a pseudoinverse \\
...
% $\mathbf{A}_y$ & $\mathbb{R}^{d\times N}$ & Obs anomalies, $\mathbf{A}_y=\mathbf{Y}_{\mathrm{stack}}-\bar{\mathbf{y}}_{\mathrm{stack}}\mathbf{1}^\top$ \\
% $\mathbf{P}_{xy}$ & $\mathbb{R}^{n\times d}$ & Cross-covariance, $\mathbf{P}_{xy}=\frac{1}{N-1}\mathbf{A}_x\mathbf{A}_y^\top$ \\
% $\mathbf{P}_{yy}$ & $\mathbb{R}^{d\times d}$ & Auto-covariance, $\mathbf{P}_{yy}=\frac{1}{N-1}\mathbf{A}_y\mathbf{A}_y^\top$ \\
...
% 	$P_{\mathcal{X}}$ & Forecast law on $\mathcal{X}$; under Gaussian modeling $P_{\mathcal{X}}=\mathcal{N}(\boldsymbol{\mu}^f,\mathbf{P}^f)$ \\
...
% \qquad
% \mathbf{A}_y := \mathbf{Y}_{\mathrm{stack}}-\bar{\mathbf{y}}_{\mathrm{stack}}\mathbf{1}^\top,
% \]
% and
% \[
% \mathbf{P}_{xy} := \frac{1}{N-1}\mathbf{A}_x\mathbf{A}_y^\top,
% \qquad
% \mathbf{P}_{yy} := \frac{1}{N-1}\mathbf{A}_y\mathbf{A}_y^\top.
% \]
% The EnDCF gain used in implementation is the \emph{empirical} gain
% \[
% \mathbf{K}^{\mathrm{EnDCF}}
...
% \mathbf{P}_{xy}\,\mathbf{P}_{yy}^{\dagger},
% \]
% where $(\cdot)^{\dagger}$ denotes the Moore--Penrose pseudoinverse (so $\mathbf{K}^{\mathrm{EnDCF}}$ is always well-defined, even when $N-1<d$).
...
% \[
% \mathbb{E}\bigl[\|\mathbf{K}^{\mathrm{EnDCF}}\|_2^2\bigr]<\infty.
% \]
